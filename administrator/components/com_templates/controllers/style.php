...
class TemplatesControllerStyle extends JControllerForm
{
	...
	public function change_parent_template($key = null, $urlVar = null)
	{
		...
		if ($task == 'save2copy')
		{
			...
			if ($checkin && $model->checkin($data[$key]) === false)
			{
				...
				$this->setRedirect(
					JRoute::_(
						'index.php?option=' . $this->option . '&view=' . $this->view_item.'&device_editing='.$device_editing
		...
		if (!$this->allowSave($data, $key))
		{
			...
			$this->setRedirect(
				JRoute::_(
					'index.php?option=' . $this->option . '&view=' . $this->view_list.'&device_editing='.$device_editing
	...
	public function change_device_editing($key = null, $urlVar = null)
	{
		// Check for request forgeries.
		JSession::checkToken() or jexit(JText::_('JINVALID_TOKEN'));
		$app   = JFactory::getApplication();
		...
		$this->setRedirect(
			JRoute::_(
				'index.php?option=' . $this->option . '&view=' . $this->view_item.'&layout=edit&id='.$id.'&device_editing='.$device_editing,false
			)
		);
		return true;
	}
