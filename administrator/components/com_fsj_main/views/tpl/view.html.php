...
class fsj_mainViewtpl extends JViewLegacy
{
	protected $form;
	protected $item;
	protected $state;
	...
	protected function addToolbar()
	{
		...
		$text = JText::_(($checkedOut ? 'FSJ_VIEW' : ($isNew ? 'FSJ_ADD' : 'FSJ_EDIT'))) . " " . JText::_('COM_fsj_main_ITEMS_tpl');
		...
		if ($isNew && (count($user->getAuthorisedCategories('com_fsj_main', 'core.create')) > 0)) {
			if ($can_save)
			{
				JToolBarHelper::apply('tpl.apply');
				JToolBarHelper::save('tpl.save');
				JToolBarHelper::save2new('tpl.save2new');
			}
			JToolBarHelper::cancel('tpl.cancel');
		}
		else {
			...
			if ($canDo->get('core.create')) {
				JToolBarHelper::save2copy('tpl.save2copy');
			}

			JToolBarHelper::cancel('tpl.cancel', 'JTOOLBAR_CLOSE');
		}
