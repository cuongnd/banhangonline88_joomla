...
class fsj_mainViewtpls extends JViewLegacy
{
	protected $items;
	...
	protected function loadFilters()
	{
		...
		$filter = array();
		$filter[] = JHTML::_('select.option', '', '- ' . JText::_('') . ' -', 'type', 'title');
		
		
		
					
			$query = 'SELECT type, title FROM #__fsj_tpl_type WHERE 1 ';
			$sort = ' ORDER BY title';
		...
		$this->filters['type'] = JHTML::_('select.genericlist',  $filter, 'filter_type', 'class="inputbox" size="1" onchange="document.adminForm.submit( );"', 'type', 'title', $this->state->get('filter.type'));
	...
	protected function addToolbar()
	{
		...
		{
			JToolBarHelper::title(JText::_('COM_FSJ_'.$admin_com.'_SHORT' ). ": " . JText::_('COM_fsj_main_ITEMS_tplS' ), $icon);
		} else {
			JToolBarHelper::title(JText::_(JRequest::getVar('option').'_SHORT' ). ": " . JText::_('COM_fsj_main_ITEMS_tplS' ), $icon);
		}



		if (($canDo->get('core.edit')) || ($canDo->get('core.edit.own'))) {
			JToolBarHelper::editList('tpl.edit');
		}
		...
		if ($this->state->get('filter.published') == -2 && $canDo->get('core.delete')) {
			JToolBarHelper::deleteList('', 'tpls.delete', 'JTOOLBAR_EMPTY_TRASH');
			JToolBarHelper::divider();
		} elseif ($canDo->get('core.edit.state')) {
			JToolBarHelper::deleteList('', 'tpls.delete', 'JACTION_DELETE');
		}
	...
	function setupOrdering()
	{
		$orders = array();
											...
											$orders['lf3 ASC'] = JText::_('FSJ_MAIN_FORM_TPL_TYPE') . " " . JText::_("JGLOBAL_ORDER_ASCENDING");
						$orders['lf3 DESC'] = JText::_('FSJ_MAIN_FORM_TPL_TYPE') . " " . JText::_("JGLOBAL_ORDER_DESCENDING");
				...
				$orders['a.title DESC'] = JText::_('JGLOBAL_TITLE') . " " . JText::_("JGLOBAL_ORDER_DESCENDING");
						...
						$orders['a.description DESC'] = JText::_('FSJ_FORM_DESCRIPTION') . " " . JText::_("JGLOBAL_ORDER_DESCENDING");
											...
											$orders['a.noedit ASC'] = JText::_('FSJ_MAIN_FORM_TPL_NOEDIT') . " " . JText::_("JGLOBAL_ORDER_ASCENDING");
						$orders['a.noedit DESC'] = JText::_('FSJ_MAIN_FORM_TPL_NOEDIT') . " " . JText::_("JGLOBAL_ORDER_DESCENDING");
