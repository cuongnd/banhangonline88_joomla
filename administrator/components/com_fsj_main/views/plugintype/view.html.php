...
class fsj_mainViewplugintype extends JViewLegacy
{
	protected $form;
	protected $item;
	protected $state;
	...
	protected function addToolbar()
	{
		...
		$text = JText::_(($checkedOut ? 'FSJ_VIEW' : ($isNew ? 'FSJ_ADD' : 'FSJ_EDIT'))) . " " . JText::_('COM_fsj_main_ITEMS_plugin_type');
		...
		if ($isNew && (count($user->getAuthorisedCategories('com_fsj_main', 'core.create')) > 0)) {
			if ($can_save)
			{
				JToolBarHelper::apply('plugintype.apply');
				JToolBarHelper::save('plugintype.save');
				JToolBarHelper::save2new('plugintype.save2new');
			}
			JToolBarHelper::cancel('plugintype.cancel');
		}
		else {
			...
			if ($canDo->get('core.create')) {
				JToolBarHelper::save2copy('plugintype.save2copy');
			}

			JToolBarHelper::cancel('plugintype.cancel', 'JTOOLBAR_CLOSE');
		}
