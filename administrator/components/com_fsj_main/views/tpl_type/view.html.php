...
class fsj_mainViewtpl_type extends JViewLegacy
{
	protected $form;
	protected $item;
	protected $state;
	...
	protected function addToolbar()
	{
		...
		$text = JText::_(($checkedOut ? 'FSJ_VIEW' : ($isNew ? 'FSJ_ADD' : 'FSJ_EDIT'))) . " " . JText::_('COM_fsj_main_ITEMS_tpl_type');
		...
		if ($isNew && (count($user->getAuthorisedCategories('com_fsj_main', 'core.create')) > 0)) {
			if ($can_save)
			{
				JToolBarHelper::apply('tpl_type.apply');
				JToolBarHelper::save('tpl_type.save');
				JToolBarHelper::save2new('tpl_type.save2new');
			}
			JToolBarHelper::cancel('tpl_type.cancel');
		}
		else {
			...
			if ($canDo->get('core.create')) {
				JToolBarHelper::save2copy('tpl_type.save2copy');
			}

			JToolBarHelper::cancel('tpl_type.cancel', 'JTOOLBAR_CLOSE');
		}
