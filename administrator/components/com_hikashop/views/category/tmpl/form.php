    ...
    <div class="span6">
        ...
        <fieldset class="adminform" id="htmlfieldset_additional">
                ...
                if (!empty($this->fields)) {
                    foreach ($this->fields as $fieldName => $oneExtraField) {
                        if (!$oneExtraField->field_backend) {
                            ?>
                            <tr>
                                <td><input type="hidden" name="data[category][<?php echo $fieldName; ?>]"
                                           value="<?php echo $this->element->$fieldName; ?>"/></td>
                            </tr>
                        <?php } else { ?>
            ...
            if ((isset($this->element->category_type) && $this->element->category_type == 'status') || (isset($this->element->category_namekey) && in_array($this->element->category_namekey, array('root', 'product', 'tax', 'status', 'created', 'confirmed', 'cancelled', 'refunded', 'shipped', 'manufacturer')))) {
                ?>
                <input type="hidden" name="data[category][category_parent_id]"
                       value="<?php echo @$this->element->category_parent_id; ?>"/>
                <?php
            } else {
        ...
        <?php if ($this->category_image) { ?>
            <fieldset class="adminform" id="htmlfieldset">
                <legend><?php echo JText::_('HIKA_IMAGE'); ?></legend>
					<span id="category_image-<?php echo @$this->element->file_id; ?>">
                        ...
                        <span
                            class="spanloading"><?php if (!empty($this->element->file_path)) echo $this->toggle->delete("category_image-" . $this->element->file_id, 'category-' . $this->element->category_id, 'file', true); ?></span><br/></span>
            ...
            <fieldset class="adminform" id="htmlfieldset">
                <legend><?php echo JText::_('Icon'); ?></legend>
					<span id="category_image-<?php echo @$this->element->icon_file_id; ?>">
						<?php echo $this->image->display(@$this->element->icon_file_path, true, "", '', '', 100, 100); ?>
                        <span
                            class="spanloading"><?php if (!empty($this->element->icon_file_id)) echo $this->toggle->delete("category_image-" . $this->element->icon_file_id, 'category_icon-' . $this->element->category_id, 'file', true); ?></span><br/></span>
