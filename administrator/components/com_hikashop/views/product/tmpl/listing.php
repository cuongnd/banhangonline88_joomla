...
$doc->addScript(JUri::root() . 'administrator/components/com_hikashop/assests/js/jquery.view_product_listing.js');
$doc->addLessStyleSheet(JUri::root() . 'administrator/components/com_hikashop/assests/less/view_product_listing.less');
?>
                                    ...
                                    <td>
                                        ...
                                        <div class="row-fluid">
                                            <div class="span6">
                                                ...
                                                ?>
                                                <?php if (!$this->config->get('category_explorer')) { ?>
                                                    <a href="<?php echo hikashop_completeLink('product&task=listing&filter_id=0'); ?>"><?php echo JText::_('ROOT'); ?>
                                                        /</a>
                                                ...
                                                <input type="text" name="search" id="search"
                                                       value="<?php echo $this->escape($this->pageInfo->search); ?>"
                                                       class="text_area" onchange="this.form.submit();"/>
                                                <button class="btn"
                                                        onclick="this.form.limitstart.value=0;this.form.submit();"><?php echo JText::_('GO'); ?></button>
                                                <button class="btn"
                                                        onclick="this.form.limitstart.value=0;document.getElementById('search').value='';this.form.submit();"><?php echo JText::_('RESET'); ?></button>
                                    </td>
                                    <td>
                                        <?php } else { ?>
                                        <div class="input-prepend input-append" style="margin-top:4px;">
                                            ...
                                            <button class="btn"
                                                    onclick="this.form.limitstart.value=0;document.getElementById('search').value='';this.form.submit();">
...
<table id="hikashop_product_listing" class="adminlist table table-striped table-hover" cellpadding="1">
    ...
    <tr>
        ...
        <th class="title"><?php
            ...
            ?></th>
        <th class="title"><?php
            echo JHTML::_('grid.sort', JText::_('HIKA_NAME'), 'b.product_name', $this->pageInfo->filter->order->dir, $this->pageInfo->filter->order->value); ?>
        ...
        <th class="title titleorder"><?php
            if ($this->doOrdering) {
                ...
                ?><a href="#"
                     title="<?php echo JText::_('CHANGE_SUB_ELEMENT_FILTER_TO_REORDER_ELEMENTS'); ?>"><?php echo JText::_('HIKA_ORDER'); ?></a><?php
            }
            ?></th>
                ...
                if ($this->doOrdering) {
                    if ($this->manage) { ?>
                        ...
                        <input type="text" name="order[]"
                               size="5" <?php if (!$this->order->ordering) echo 'disabled="disabled"' ?>
                               value="<?php echo $row->ordering; ?>" class="text_area" style="text-align: center"/>
                    <?php } else {
                        echo $row->ordering;
                    }
                } else {
                    ?><a href="#" title="<?php echo JText::_('CHANGE_SUB_ELEMENT_FILTER_TO_REORDER_ELEMENTS'); ?>"><img
                        src="<?php echo HIKASHOP_IMAGES; ?>delete2.png" alt="<?php echo JText::_('HIKA_DELETE'); ?>">
                    </a><?php
                }
                ?></td>
            <td align="center"><?php
                if ($this->manage) {
                    ?><span id="<?php echo $publishedid ?>"
                            class="spanloading"><?php echo $this->toggleClass->toggle($publishedid, (int)$row->product_published, 'product') ?></span><?php
                } else {
...
<script type="text/javascript">
    jQuery(document).ready(function ($) {
        $("#page-product").view_product_listing({
