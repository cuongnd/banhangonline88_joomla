...
class productauctionViewProductauction extends hikaauctionView {
	...
	public function show_block_auction_history($params = null) {
		...
		if(!empty($product_id)) {
			...
			$auctionHistory = $auctionClass->getAuctionHistory($product_id);
		...
		$this->assignRef('auctionHistory', $auctionHistory);
	}
	...
	public function shop_form($params = null) {
		...
		if($product_auction > 0) {
			...
			$query = $dbHelper->getQuery(true);
			$query->select('COUNT(op.order_product_quantity) as orders, SUM(op.order_product_quantity) as sales')
