...
class JButtonPophelp extends JButton {
	...
	public function fetchButton($type = 'Pophelp', $namekey = '', $id = 'pophelp') {
		...
		$url = HIKAAUCTION_HELPURL.$namekey.'&level='.$level;
		$js = '
function displayDoc(){
	...
	return false;
		...
		if(!HIKAAUCTION_BACK_RESPONSIVE)
			return '<a href="' . $url . '" target="_blank" onclick="return displayDoc();" class="toolbar"><span class="icon-32-help" title="' . JText::_('HKA_HELP', true) . '"></span>' . JText::_('HKA_HELP') . '</a>';
		return '<button class="btn btn-small" onclick="return displayDoc();"><i class="icon-help"></i> ' . JText::_('HKA_HELP') . '</button>';
	}
