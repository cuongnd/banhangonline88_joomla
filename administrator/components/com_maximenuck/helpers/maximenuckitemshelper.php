...
class MaximenuckItemsHelper
{
	...
	public static function addSubmenu($vName)
	{
		JHtmlSidebar::addEntry(
			...
			'index.php?option=com_maximenuck&view=menus',
			$vName == 'menus'
		);
		JHtmlSidebar::addEntry(
			JText::_('COM_MENUS_SUBMENU_ITEMS'),
			'index.php?option=com_maximenuck&view=items',
			$vName == 'items'
		);
	}
	...
	public static function getActions($parentId = 0)
	{
		...
		{
			$assetName = 'com_maximenuck.item.'.(int) $parentId;
		}

		$actions = JAccess::getActions('com_maximenuck');
	...
	static public function getAssociations($pk)
	{
		...
		$query->innerJoin('#__associations as a ON a.id=m.id AND a.context='.$db->quote('com_maximenuck.item'));
