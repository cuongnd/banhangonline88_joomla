...
require_once JPATH_COMPONENT.'/helpers/maximenuckitemshelper.php';
 ...
 * @since       1.6
 */
class MaximenuckModelItem extends JModelAdmin
{
	/**
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_maximenuck.item.'.(int) $record->id);
		}
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		$user = JFactory::getUser();
		if (!$user->authorise('core.create', 'com_maximenuck'))
		{
	...
	protected function batchMove($value, $pks, $contexts)
	{
		...
		if (!$user->authorise('core.edit', 'com_maximenuck'))
		{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_maximenuck.item', 'item', array('control' => 'jform', 'load_data' => $loadData), true);
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		return array_merge((array) $this->getItem(), (array) JFactory::getApplication()->getUserState('com_maximenuck.edit.item.data', array()));
	}
	...
	public function getItem($pk = null)
	{
		...
		if ($link = $this->getState('item.link'))
		{
			...
			if (MaximenuckItemsHelper::getLinkKey($table->link) != MaximenuckItemsHelper::getLinkKey($link))
			{
				$table->link = $link;
			}
		}
		...
		if ($assoc)
		{
			if ($pk != null)
			{
				$result->associations = MaximenuckItemsHelper::getAssociations($pk);
			}
			else {
	...
	protected function populateState()
	{
		...
		if (!($parentId = $app->getUserState('com_maximenuck.edit.item.parent_id')))
		{
			$parentId = $app->input->getInt('parent_id');
		}
		$this->setState('item.parent_id', $parentId);

		$menuType = $app->getUserState('com_maximenuck.edit.item.menutype');
		...
		if (!($type = $app->getUserState('com_maximenuck.edit.item.type')))
		{
		...
		if ($link = $app->getUserState('com_maximenuck.edit.item.link'))
		{
	...
	public function save($data)
	{
		...
		$isNew = true;
		$db    = $this->getDbo();
		// JTable::addIncludePath(JPATH_ADMINISTRATOR.'/components/com_maximenuck/tables');
		$table = $this->getTable();
		...
		if ($assoc)
		{
			...
			$query->where('context='.$db->quote('com_maximenuck.item'));
			...
			if (!$all_language && count($associations) > 1)
			{
				...
				foreach ($associations as $tag => $id)
				{
					$query->values($id.','.$db->quote('com_maximenuck.item').','.$db->quote($key));
				}
				$db->setQuery($query);
