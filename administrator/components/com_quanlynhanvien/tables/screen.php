 ...
 */
class JTablescreen extends JTable
{
	/**
	 * Constructor
	 *
	...
	{
		parent::__construct('#__screen', 'id', $db);

		JTableObserverTags::createObserver($this, array('typeAlias' => 'com_screen.article'));
		JTableObserverscreenhistory::createObserver($this, array('typeAlias' => 'com_screen.article'));
	...
	protected function _getAssetName()
	{
		...
		return 'com_screen.article.' . (int) $this->$k;
	}
	 ...
	 * @param   integer  $id     The id (optional) of the screen.
	 *
	 * @return  integer
	 *
	 * @since   11.1
	...
	public function check()
	{
		if (trim($this->title) == '')
		{
			$this->setError(JText::_('COM_screen_WARNING_PROVIDE_VALID_NAME'));
		...
		if (!$this->id)
		{
			...
			if (!$this->getRules())
			{
				$rules = $this->getDefaultAssetValues('com_screen');
				$this->setRules($rules);
			}
		}
	...
	public function store($updateNulls = false)
	{
		...
		$table = JTable::getInstance('screen', 'JTable', array('dbo' => $this->getDbo()));
