...
class fsj_transmanViewpackage extends JViewLegacy
{
	protected $form;
	protected $item;
	protected $state;
	...
	protected function addToolbar()
	{
		...
		$text = JText::_(($checkedOut ? 'FSJ_VIEW' : ($isNew ? 'FSJ_ADD' : 'FSJ_EDIT'))) . " " . JText::_('COM_fsj_transman_ITEMS_transman_package');
		...
		if ($isNew && (count($user->getAuthorisedCategories('com_fsj_transman', 'core.create')) > 0)) {
			if ($can_save)
			{
				...
				JToolBarHelper::save2new('package.save2new');
			}
			...
			if ($canDo->get('core.create')) {
				JToolBarHelper::save2copy('package.save2copy');
			}
			...
			JToolBarHelper::cancel('package.cancel', 'JTOOLBAR_CLOSE');
				...
				JToolBarHelper::custom('NONE', 'plus', 'plus', 'FSJ_TM_ADD', false);
