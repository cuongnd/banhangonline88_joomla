...
class TM_Auto_Google_Hack
{
	...
	{	
		...
		$tag = "xxyyxxyyxx";
		$url = "https://translate.google.com/translate_a/single?client=t&dt=bd&dt=ex&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=ss&dt=t&dt=at&ie=UTF-8&oe=UTF-8&rom=0&ssel=0&tsel=4&kc=0&tk=952859.568363&";
		$result = array();
		$to_translate = array();
		...
		$params = array();
		$params['sl'] = $this->getLangCode($source);
		$params['tl'] = $this->getLangCode($dest);
		$params['hl'] = $this->getLangCode($source);
		$params['q'] = $text;
		$parts = array();
		...
		//exit;
		$page = substr($page, 1, strlen($page) - 2);
		$result = $this->parseResult($page);
		//print_p($result);
		$current_key = -1;
		$output = "";
