...
class plgSystemHikashopsocial extends JPlugin
{
    ...
    var $params = array();
    var $added_script = false;
    function __construct(&$subject, $params)
    {
        parent::__construct($subject, $params);
    }
    function getFacebookShareButton($plugin, $link = "")
    {
        $element = $this->_getElementInfo();
        if (empty($element))
            return '';
        if (!empty($element->url_canonical))
            $link_article = hikashop_cleanURL($element->url_canonical);
        else
            $link_article = hikashop_currentURL('', false);
        if ($link != "") {
            $link_article = $link;
        }
        $title = $element->name;
        $html = array();
        $buttonType = $plugin->params->get('facebook_share_button_type');
        $html[] = '<div class="pull-left bt-social-share-button bt-facebook-share-button">';
        if ($buttonType) {
    ...
    function kashopsocial_product_listing($link, $image_url, $title)
    {
        ...
        $html = array(
            $this->_addTwitterButton($this, $link, $image_url, $title),
            $this->getFacebookShareButton($this, $link, $image_url, $title),
            $this->_addPinterestButton($this, $link, $image_url, $title),
            $this->_addGoogleButton($this, $link, $image_url, $title),
            $this->_addAddThisButton($this, $link, $image_url, $title),
            $this->_addFacebookButton($this, $link, $image_url, $title)
        );
    ...
    function onAfterRender()
    {
        ...
        if (!isset($plugin->params['fb_tag']))
            $plugin->params['fb_tag'] = 'iframe';
        $matches_replate = $matches;
        foreach ($matches_replate as &$item) {
            preg_match_all('/{hikashop_social link="(.+)"}/', $item, $link);
            $link = $link[1][0];
            $link = JUri::root() . $link;
            preg_match_all('/{hikashop_social image_url="(.+)"}/', $item, $image_url);
            $image_url = $image_url[1][0];
            $image_url = JUri::root() . $image_url;
            preg_match_all('/{hikashop_social title="(.+)"}/', $item, $title);
            $title = $title[1][0];
            $html = array(
                $this->_addTwitterButton($this, $link, $image_url, $title),
                $this->getFacebookShareButton($this, $link, $image_url, $title),
...
(function(d,s,id) {
    ...
    function _addAddThisButton(&$plugin, $link = "")
    {
    ...
    function _addGoogleButton(&$plugin, $link = "")
    {
    ...
    function _addPinterestButton(&$plugin, $link = "", $image_url = "", $title = "")
    {
        ...
        return '<span class="hikashop_social_pinterest' . $c . '"><a target="_blank" href="//pinterest.com/pin/create/button/?url=' . urlencode($url) . '&media=' . urlencode($image_url) . '&description=' . rawurlencode($description) . '" class="pin-it-button" count-layout="' . $count . '"><img border="0" src="//assets.pinterest.com/images/PinExt.png" title="Pin It" /></a></span>';
    }
    function _addTwitterButton(&$plugin, $link = "")
    {
...
function twitterPop(str) {
    ...
    function _addFacebookButton(&$plugin, $link = "")
    {
        if (empty($plugin->params['display_fb']))
            return;
        $element = $this->_getElementInfo();
