...
class plgAcymailingGeolocation extends JPlugin
{
	...
	function onAcyDisplayFilters(&$type,$context="massactions"){
		...
		$jsOnChange .= "displayCondFilter('displayValues', 'toChange__num__',__num__,'geolocMap='+searchMap+'&displayCond='+displayVal); ";
	 ...
	 function onAcyTriggerFct_displayValues(){
		...
		$geolocMap = JRequest::GetVar('geolocMap');
		$displayCond = JRequest::getVar('displayCond');

		$emptyInputReturn = '<input onchange="countresults('.$num.')" class="inputbox" type="text" name="filter['.$num.'][geolocfield][value]" id="filter'.$num.'geolocfieldvalue" style="width:200px" value="">';
		...
		if($displayCond == 'ok' && in_array($geolocMap, $listType)){
			$db = JFactory::getDBO();
			...
			$db->setQuery($query);
	 		...
	 		return JHTML::_('select.genericlist', $valueProp, "filter[$num][geolocfield][value]", 'onchange="countresults('.$num.')" class="inputbox" size="1" style="width:200px"', 'valueProp', 'valueProp', null, 'filter'.$num.'geolocfieldvalue');
		} else{ // free input
