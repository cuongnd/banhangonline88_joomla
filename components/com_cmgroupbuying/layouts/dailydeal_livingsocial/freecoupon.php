...
else
{
	...
	?>
	<script>
		jQuery("#cm_deals_stretch_image").css("background-image","url(<?php echo JURI::root() . $coupon['background_image']; ?>)");
	</script>
	<?php
	endif;
				...
				;(function($) {
					$(function() {
						$('.buy_button').bind('click', function(e) {
							e.preventDefault();
							window.open('index.php?option=com_cmgroupbuying&controller=freecoupon&task=download&coupon_id=<?php echo $coupon['id']; ?>', '_blank');
						});
					});
				})(jQuery);
			</script>
...
<?php endif; ?>
	<div class="deal_section clearfix">
		<div class="deal_section_left">
			<?php
			if(count($couponImages) == 1)
			{
				echo '<div class="deal_photo"><img src="' . $couponImages[0] . '" width="' . $imageWidth . 'px" /></div>';
			}
			elseif(count($couponImages) > 1)
			{
		...
		<div class="deal_section_right">
			<div class="deal_name">
				<h1><?php echo $coupon['name']; ?></h1>
				<p class="deal_short_description"><?php echo $coupon['short_description']; ?></p>
			</div>
			<div class="price_block">
				<div id="coupon_code_block">
					<div class="price">
						<?php echo $couponDiscount; ?>
					</div>
					<div class="buy_button">
						<?php if($startDate > $today): ?>
						<div class="cm_button upcoming_button"><span><?php echo JText::_('COM_CMGROUPBUYING_UPCOMING_BUTTON'); ?></span></div>
						<?php elseif($endDate < $today): ?>
						<div class="cm_button expired_button"><span><?php echo JText::_('COM_CMGROUPBUYING_EXPIRED_BUTTON'); ?></span></div>
						<?php else: ?>
							<?php if($coupon['type'] == "printable"): ?>
							<div class="cm_button available_button"><span><?php echo JText::_('COM_CMGROUPBUYING_FREE_COUPON_PRINT_BUTTON'); ?></span></div>
							<?php elseif($coupon['type'] == "code"): ?>
							<div class="cm_button available_button"><span><?php echo JText::_('COM_CMGROUPBUYING_FREE_COUPON_GET_CODE_BUTTON'); ?></span></div>
							<?php endif; ?>
						<?php endif; ?>
					</div>
				</div>
				<ul class="clearfix deal_detail_info coupon_detail_info">
					<li class="purchased">
						<div class="value"><?php echo $coupon['view']; ?></div>
						<div class="label"><?php echo str_replace("%s", "", JText::_('COM_CMGROUPBUYING_FREE_COUPON_VIEW_MESSAGE')); ?></div>
					</li>
					<li class="last">
						<div class="value">
