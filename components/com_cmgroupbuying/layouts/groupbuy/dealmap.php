		...
		function load() {
			...
			downloadUrl("index.php?option=com_cmgroupbuying&controller=dealmap&task=generateXML", function(data) {
				...
				for (var i = 0; i < markers.length; i++) {
					...
					html = html + "</table>";
					html = html + "</td></tr>";
					html = html + "<tr><td><div class='deal_bought'>" + bought + " <?php echo JText::_('COM_CMGROUPBUYING_BOUGHT'); ?></div></td><td><div class='pull-right'><a class='btn btn-primary' href='" + link + "' target='_blank'><?php echo JText::_('COM_CMGROUPBUYING_DEAL_MAP_MORE_INFO'); ?></a></div></td></tr>";
					html = html + "</table>";
...
<div id="map" style="width: 100%; height: <?php echo $this->configuration['deal_map_height']; ?>px"></div>