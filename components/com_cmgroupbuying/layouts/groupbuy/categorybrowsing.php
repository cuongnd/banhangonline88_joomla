...
<script>
	...
	{
		jQuery("#deal_column").empty().html('<div style="height: 100px" class="loading"></div>');
		jQuery.ajax({
			url: "index.php",
			type: "post",
	...
	var customIcons = {
		...
		downloadUrl("index.php?option=com_cmgroupbuying&controller=dealmap&task=generateXML", function(data) {
			...
			for (var i = 0; i < markers.length; i++) {
				...
				html = html + '<tr><td class="deal_bought" >' + bought + ' <?php echo JText::_('COM_CMGROUPBUYING_BOUGHT'); ?></td><td class="pull-right"><a class="btn btn-primary" href="' + link + '" target="_blank"><?php echo JText::_('COM_CMGROUPBUYING_DEAL_MAP_MORE_INFO'); ?></a></td></tr>';
				html = html + '</table>';
...
<div id="cm_category_browsing" class="row-fluid">
	<div class="category_navigation span12">
		<div class="row-fluid">
			<div class="span3 category_column" id="category_column">
			...
			<div class="span3  deal_column" id="deal_column"></div>
			<div class="span6 deal_map_container" style="height: <?php echo $this->configuration['deal_map_height']; ?>px">
				<div id="map"></div>
			</div>
		</div>
	</div>
