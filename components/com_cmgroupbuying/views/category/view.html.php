...
class CMGroupBuyingViewCategory extends JViewLegacy
{
	function display($tpl = null)
	{
		...
		$categoryId = $jinput->get('id', 0, 'int');
		...
		$deals = JModelLegacy::getInstance('Deal', 'CMGroupBuyingModel')->getLimit(false, false, false, $categoryId, null, $locationString);
		$this->assignRef('deals', $deals);

		//getPagination($numberOfDeals = null, $active = false, $upcoming = false, $expired = false, $categoryId = null, $locationString = '')
		$pageNav = JModelLegacy::getInstance('Deal', 'CMGroupBuyingModel')->getPagination(null, false, false, false, $categoryId, null, $locationString);
		$this->assignRef('pageNav', $pageNav);

		$pageTitle = JFactory::getDocument()->getTitle();
		$this->assignRef('pageTitle', $pageTitle);

		$noDeal = JText::sprintf('COM_CMGROUPBUYING_CATEGORY_NO_DEAL', $category['name']);
