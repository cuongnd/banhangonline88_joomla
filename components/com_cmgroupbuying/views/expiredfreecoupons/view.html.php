...
class CMGroupBuyingViewExpiredFreeCoupons extends JViewLegacy
{
	function display($tpl = null)
	{
		...
		if($locationId > 0)
		{
			$locationString = CMGroupBuyingHelperFreeCoupon:: getFreeCouponsInLocation($locationId);
		}

		$view = JFactory::getApplication()->input->get('view', '', 'word');
		$this->assignRef('view', $view);

		$coupons = JModelLegacy::getInstance('FreeCoupon', 'CMGroupBuyingModel')->getLimit($locationString, 'expired');
		$this->assignRef('coupons', $coupons);

		$pageNav = JModelLegacy::getInstance('FreeCoupon', 'CMGroupBuyingModel')->getPagination(null, $locationString, 'expired');
		...
		$this->_layout = "expiredfreecoupons";
		parent::display($tpl);
	}
}