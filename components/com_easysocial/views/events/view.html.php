...
class EasySocialViewEvents extends EasySocialSiteView
{
    ...
    public function display($tpl = null)
    {
        ...
        $options = array(
            ...
            'ordering' => $ordering,
            'type' => $this->my->isSiteAdmin() ? 'all' : array(SOCIAL_EVENT_TYPE_PUBLIC, SOCIAL_EVENT_TYPE_PRIVATE),
        ...
        if ($filter === 'date') {
            ...
            list($nowYMD, $nowHMS) = explode(' ', $now->toSql());
            ...
            switch ($mode) {
                ...
                case 2:
                    ...
                    $nextDate = FD::date($start)->modify('+1 month')->format('Y-m', true);
                    $prevDate = FD::date($start)->modify('-1 month')->format('Y-m', true);
                    ...
                    if ($nextDate == $currentMonth) {
                        ...
                        $nextTitle = JText::sprintf('COM_EASYSOCIAL_PAGE_TITLE_EVENTS_FILTER_DATE', FD::date($nextDate . '-01')->format(JText::_($dateFormat), true));
                    }

                    if ($prevDate == $currentMonth) {
                        $prevTitle = JText::_('COM_EASYSOCIAL_PAGE_TITLE_EVENTS_FILTER_MONTH');
                    } else {
                        $prevTitle = JText::sprintf('COM_EASYSOCIAL_PAGE_TITLE_EVENTS_FILTER_DATE', FD::date($prevDate . '-01')->format(JText::_($dateFormat), true));
                    }
                break;
                ...
                case 3:
                    ...
                    $nextDate = FD::date($start)->modify('+1 day')->format('Y-m-d', true);
                    $prevDate = FD::date($start)->modify('-1 day')->format('Y-m-d', true);

                    $nextLinkOptions = array('filter' => 'date');
                    $prevLinkOptions = array('filter' => 'date');

                    if ($nextDate == $currentDay) {
                        $nextTitle = JText::_('COM_EASYSOCIAL_PAGE_TITLE_EVENTS_FILTER_TODAY') . ' - ' . FD::date($nextDate)->format(JText::_($dateFormat), true);
                    } elseif ($nextDate == $nextDay) {
                        $nextTitle = JText::_('COM_EASYSOCIAL_PAGE_TITLE_EVENTS_FILTER_TOMORROW') . ' - ' . FD::date($nextDate)->format(JText::_($dateFormat), true);

                        $nextLinkOptions['date'] = $nextDate;
                    } else {
                        $nextTitle = JText::sprintf('COM_EASYSOCIAL_PAGE_TITLE_EVENTS_FILTER_DATE', FD::date($nextDate)->format(JText::_($dateFormat), true));
                    ...
                    if ($prevDate == $currentDay) {
                        $prevTitle = JText::_('COM_EASYSOCIAL_PAGE_TITLE_EVENTS_FILTER_TODAY') . ' - ' . FD::date($prevDate)->format(JText::_($dateFormat), true);
                    } elseif ($prevDate == $nextDay) {
                        $prevTitle = JText::_('COM_EASYSOCIAL_PAGE_TITLE_EVENTS_FILTER_TOMORROW') . ' - ' . FD::date($prevDate)->format(JText::_($dateFormat), true);

                        $prevLinkOptions['date'] = $prevDate;
                    } else {
                        $prevTitle = JText::sprintf('COM_EASYSOCIAL_PAGE_TITLE_EVENTS_FILTER_DATE', FD::date($prevDate)->format(JText::_($dateFormat), true));
        ...
        $total = $model->getTotalEvents(array('state' => SOCIAL_STATE_PUBLISHED, 'type' => $this->my->isSiteAdmin() ? 'all' : array(SOCIAL_EVENT_TYPE_PUBLIC, SOCIAL_EVENT_TYPE_PRIVATE), 'start-after' => FD::date()->toSql()));
        $totalFeaturedEvents = $model->getTotalEvents(array('state' => SOCIAL_STATE_PUBLISHED,'featured' => true, 'start-after' => FD::date()->toSql()));
        $totalCreatedEvents = $model->getTotalEvents(array('state' => SOCIAL_STATE_PUBLISHED, 'creator_uid' => $this->my->id, 'creator_type' => SOCIAL_TYPE_USER, 'type' => 'all', 'start-after' => FD::date()->toSql()));
        $totalInvitedEvents = $model->getTotalEvents(array('state' => SOCIAL_STATE_PUBLISHED, 'guestuid' => $this->my->id, 'gueststate' => SOCIAL_EVENT_GUEST_INVITED, 'type' => 'all', 'start-after' => FD::date()->toSql()));

        $now = FD::date();
        $week1 = FD::date($now->toUnix() + 60*60*24*7);
        $week2 = FD::date($now->toUnix() + 60*60*24*14);

        $totalWeek1Events = $model->getTotalEvents(array('state' => SOCIAL_STATE_PUBLISHED, 'type' => $this->my->isSiteAdmin() ? 'all' : array(SOCIAL_EVENT_TYPE_PUBLIC, SOCIAL_EVENT_TYPE_PRIVATE), 'start-after' => $now->toSql(), 'start-before' => $week1->toSql()));
        $totalWeek2Events = $model->getTotalEvents(array('state' => SOCIAL_STATE_PUBLISHED, 'type' => $this->my->isSiteAdmin() ? 'all' : array(SOCIAL_EVENT_TYPE_PUBLIC, SOCIAL_EVENT_TYPE_PRIVATE), 'start-after' => $now->toSql(), 'start-before' => $week2->toSql()));

        $totalPastEvents = $model->getTotalEvents(array('state' => SOCIAL_STATE_PUBLISHED, 'type' => $this->my->isSiteAdmin() ? 'all' : array(SOCIAL_EVENT_TYPE_PUBLIC, SOCIAL_EVENT_TYPE_PRIVATE), 'start-before' => $now->toSql()));

        $totalTodayEvents = $model->getTotalEvents(array('state' => SOCIAL_STATE_PUBLISHED, 'type' => $this->my->isSiteAdmin() ? 'all' : array(SOCIAL_EVENT_TYPE_PUBLIC, SOCIAL_EVENT_TYPE_PRIVATE), 'start-after' => $now->format('Y-m-d 00:00:00', true), 'start-before' => $now->format('Y-m-d 23:59:59', true)));

        $tomorrow = FD::date()->modify('+1 day');

        $this->set('tomorrow', $tomorrow->format('Y-m-d', true));

        $totalTomorrowEvents = $model->getTotalEvents(array('state' => SOCIAL_STATE_PUBLISHED, 'type' => $this->my->isSiteAdmin() ? 'all' : array(SOCIAL_EVENT_TYPE_PUBLIC, SOCIAL_EVENT_TYPE_PRIVATE), 'start-after' => $tomorrow->format('Y-m-d 00:00:00', true), 'start-before' => $tomorrow->format('Y-m-d 23:59:59', true)));
        ...
        $totalMonthEvents = $model->getTotalEvents(array('state' => SOCIAL_STATE_PUBLISHED, 'type' => $this->my->isSiteAdmin() ? 'all' : array(SOCIAL_EVENT_TYPE_PUBLIC, SOCIAL_EVENT_TYPE_PRIVATE), 'start-after' => $now->format('Y-m-01 00:00:00', true), 'start-before' => $now->format('Y-m-' . $currentMonthMaxDay . ' 23:59:59', true)));
        $totalYearEvents = $model->getTotalEvents(array('state' => SOCIAL_STATE_PUBLISHED, 'type' => $this->my->isSiteAdmin() ? 'all' : array(SOCIAL_EVENT_TYPE_PUBLIC, SOCIAL_EVENT_TYPE_PRIVATE), 'start-after' => $now->format('Y-01-01 00:00:00', true), 'start-before' => $now->format('Y-12-31 23:59:59', true)));
    ...
    public function create()
    {
        ...
        if (!$this->my->isSiteAdmin() && $this->my->getAccess()->exceeded('events.limit', $this->my->getTotalCreatedEvents())) {
