...
var deleteSessionPromise=$.Deferred(function(defer){$.ajax({type:"POST",url:jsonLiveSite,dataType:'json',context:this,data:{task:'webrtc.deleteEntity',ids:remotePeer}}).done(function(response,textStatus,jqXHR){if(!response.storing.status){defer.reject(response.storing.exception_message,textStatus);return!1}
...
candidatesCounter++;peerConnection.iceCandidatesSent=!1;if(debugLogging){console.log('Event candidate found: '+event.candidate.candidate)}}else if(!event.candidate||peerConnection.iceGatheringState==='complete'){if(!peerConnection.iceCandidatesSent){signalingChannel('icecandidate',peerCandidates);peerConnection.iceCandidatesSent=!0;if(debugLogging){console.log('icecandidates sent successfully using standard complete event')}}}};var checkIceCandidatesSent=function(){setTimeout(function(){if(peerConnection){if(!peerConnection.iceCandidatesSent){signalingChannel('icecandidate',peerCandidates);peerConnection.iceCandidatesSent=!0;if(debugLogging){console.log('icecandidates sent successfully using polyfill')}}}},1000)};var signalingChannel=function(dataType,data){var postData={task:'webrtc.saveEntity',peer2:remotePeer,caller:(bindContext.caller?1:0),videocam:thispeerVideoCamState};switch(dataType){case'sdp':$.extend(postData,{'sdp':JSON.stringify(data)});break;case'icecandidate':$.extend(postData,{'icecandidate':JSON.stringify(data)});break}
var signalingChannelPromise=$.Deferred(function(defer){$.ajax({type:"POST",url:jsonLiveSite,dataType:'json',context:this,data:postData}).done(function(response,textStatus,jqXHR){if(!response.storing.status){defer.reject(response.storing.exception_message,textStatus,response.storing);return!1}
defer.resolve(response.storing)}).fail(function(jqXHR,textStatus,errorThrown){var genericStatus=textStatus[0].toUpperCase()+textStatus.slice(1);defer.reject('-'+genericStatus+'- '+errorThrown,null,{})})}).promise();signalingChannelPromise.then(function(responseData){promiseResolved=!0;if(debugLogging){console.log(dataType+' sent succesfully using signaling channel')}},function(errorText,error,exception){if(peerConnection){if(exception.usermessage){showRTCMessages('jchat_exceptions','jchat_icon_error','jchat_tooltip_error',errorText,'jchat_exceptions_closer')}else{showRTCMessages('jchat_exceptions','jchat_icon_error','jchat_tooltip_error',jchat_error_creating_connection,'jchat_exceptions_closer')}
peerConnection.close();peerConnection=null;peerCandidates=new Array();candidatesCounter=0}
bindContext.caller=!1;bindContext.callee=!1;promiseResolved=!1;resetSounds();resetButtons();resetVideo();resetBandWidth();if(debugLogging){console.log(dataType+' error using signaling channel: '+errorText)}})};this.initializeVideo=function($container,otherPeerIdentifier,otherPeerName,thisPeerName){if(!supportFullWebRTC){showFallback($container);return!1}
...
var loadTimeSecs=(window.performance.timing.responseEnd-window.performance.timing.navigationStart)/1000;$.each(webCamConstraints,function(resolution,constraint){if(resolution=='Auto'){return!0}
var bandwidthInterval=connectionSpeedConstraints[resolution];if(loadTimeSecs>=bandwidthInterval.minValue&&loadTimeSecs<bandwidthInterval.maxValue){webCamConstraintsSelect.val(resolution);webCamQuality=resolution;return!1}})};this.setListeningData=function(data,realtimeOptions){if(!supportFullWebRTC){if(debugLogging){console.log('No WebRTC support detected for this device browser')}
return!1}
callStatus=data.call_status;if(data.sdp&&data.icecandidate){receivedSdpMessage=data.sdp;receivedICECandidates=data.icecandidate;if(!this.caller){this.callee=!0;JChatNotifications.playRingingWebrtcCall(!1,options.ringingTone);if(!realtimeOptions.tabFocused){$('title').text(jchat_newvideocall_tab)}
...
$('#jchat_webrtc_camquality').attr('disabled',!0);if(debugLogging){console.log('New incoming call arrived')}}else{$('#jchat_start_accept_call',$tooltipContainer).removeClass('jchat_ringing').addClass('jchat_disabled');if(ringingInterval){clearInterval(ringingInterval);ringingInterval=null}
...
if(promiseResolved&&this.caller&&!data.caller_peer_state){resetButtons();resetBandWidth();resetSounds();JChatNotifications.playEndWebrtcCall();resetVideo();resetTooltips();this.caller=!1;promiseResolved=!1;if(peerConnection){peerConnection.close();peerConnection=null;peerCandidates=new Array();candidatesCounter=0}
...
if(this.callee&&!callStatus){resetButtons();resetBandWidth();resetSounds();JChatNotifications.playEndWebrtcCall();resetVideo();resetTooltips();this.callee=!1;promiseResolved=!1;if(!realtimeOptions.tabFocused){$('title').text(realtimeOptions.tabTitle)}
...
$('#jchat_webrtc_camquality').attr('disabled',!0);$('#jchat_start_accept_call span.text',$tooltipContainer).text(jchat_accept_call);$('#jchat_end_call span.text',$tooltipContainer).text(jchat_decline_call);$('#jchat_end_call',$tooltipContainer).removeClass('jchat_disabled');if(ringingInterval){clearInterval(ringingInterval)}
...
var isFirefox=!1;if(navigator.mozGetUserMedia){window.RTCSessionDescription=window.RTCSessionDescription||window.mozRTCSessionDescription;window.RTCIceCandidate=window.RTCIceCandidate||window.mozRTCIceCandidate;isFirefox=!0}
debugLogging=options.debugEnabled;jsonLiveSite=options.jsonLiveSite;window.AudioContext=window.AudioContext||window.webkitAudioContext;thispeerVideoCamState=$.jStorage.get('jchat_webrtc_videocam',1);webCamQuality=$.jStorage.get('jchat_webrtc_videocam_quality',!0);JChatNotifications.setAudioStatus(options.audiostatus);if(isFirefox){webCamConstraints=webCamNewConstraints}
