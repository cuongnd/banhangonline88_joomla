...
?>
<section class="discuss-post-polls">
	...
	<div class="discussPolls<?php echo $isPollLocked ? ' alert' : '';?> <?php echo $unique_poll_id; ?>">
		<!-- @php START polls new dom -->
		<?php if( $pollQuestion->title ) { ?>
		<p>
			<strong><?php echo $this->escape( $pollQuestion->title ); ?></strong>
		</p>
		<?php } ?>

		<div class="poll-locked" id="poll_notice_<?php echo $post->id;?>" style="display:<?php echo ( $isPollLocked ) ? 'block' : 'none'; ?>">
		...
		<ul class="reset-ul">
			...
			?>
				<li class="pollAnswer pollAnswerItem-<?php echo $poll->id; ?>" data-id="<?php echo $poll->id; ?>">
					<div class="<?php echo ( $pollQuestion && $pollQuestion->multiple ) ? 'checkbox' : 'radio' ?>">
						<span class="poll-count float-r muted" style="margin: 0 0 0 20px">
						...
						<label for="poll-count-<?php echo $poll->id; ?>" title="<?php echo $this->escape($poll->get( 'value' )); ?>">
					...
					<div class="progress progress-striped">
						<div style="width: <?php echo $poll->getPercentage( $totalVoted ); ?>%;" class="progress-bar progress-bar-success pollGraph"></div>
					</div>
				</li>
			<?php } ?>
		</ul>
