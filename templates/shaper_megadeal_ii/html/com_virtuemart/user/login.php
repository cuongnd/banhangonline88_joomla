...
<?php   }
    ...
    <form id="com-form-login" class="call-for-price-login" action="<?php echo JRoute::_($url, $this->useXHTML, $this->useSSL); ?>" method="post" name="com-login" >
      <fieldset class="userdata">
        <?php if (!$this->from_cart ) { ?>
        <div>
          <h2 class="title"><?php echo vmText::_('MOD_JLOGIN_TITLE'); ?></h2>
        </div>
        <?php } else { ?>
        ...
        <div class="" id="com-form-login-remember">
          <input type="submit" name="Submit" class="btn btn-success" value="<?php echo
          vmText::_('COM_VIRTUEMART_LOGIN') ?>" />       
        </div>
        <div class="clr"></div>      
      </fieldset>
      <div class="clr"></div>
