(function (){function preventDefault(){this.returnValue=false}function stopPropagation(){this.cancelBubble=true}var Oby={version:20160107,ajaxEvents:{},hasClass:function (o,n){if (o.className=='') return false;var reg=new RegExp("(^|\\s+)" + n + "(\\s+|$)");return reg.test(o.className)},addClass:function (o,n){if (!this.hasClass(o,n)){if (o.className==''){o.className=n}else{o.className +=' ' + n}}},trim:function (s){return (s ? '' + s:'').replace(/^\s*|\s*$/g,'')},removeClass:function (e,c){var t=this;if (t.hasClass(e,c)){var cn=' ' + e.className + ' ';e.className=t.trim(cn.replace(' ' + c + ' ',' '))}},addEvent:function (d,e,f){if (d.attachEvent) d.attachEvent('on' + e,f);else if (d.addEventListener) d.addEventListener(e,f,false);else d['on' + e]=f;return f},removeEvent:function (d,e,f){try{if (d.detachEvent) d.detachEvent('on' + e,f);else if (d.removeEventListener) d.removeEventListener(e,f,false);else d['on' + e]=null}},cancelEvent:function (e){if (!e){e=window.event;if (!e) return false}if (e.stopPropagation) e.stopPropagation();else e.cancelBubble=true;if (e.preventDefault) e.preventDefault();else e.returnValue=false;return false},fireEvent:function (obj,e,data){var d=document,evt=null;if (document.createEvent){evt=d.createEvent('HTMLEvents');evt.initEvent(e,false,true);if (data) evt.data=data;obj.dispatchEvent(evt);return}if (data && d.createEventObject){evt=d.createEventObject();evt.data=data;obj.fireEvent('on' + e,evt);return}obj.fireEvent('on' + e)},fireAjax:function (name,params){var t=this,ev;if (t.ajaxEvents[name]===undefined) return false;for (var e in t.ajaxEvents[name]){if (e !='_id'){ev=t.ajaxEvents[name][e];ev(params)}}return true},registerAjax:function (name,fct){var t=this;if (typeof(name)=='object'){var r=[];for (var k=name.length-1;k>=0;k--){r[r.length]=t.registerAjax(name[k],fct)}return r}if (t.ajaxEvents[name]===undefined) t.ajaxEvents[name]={'_id':0};var id=t.ajaxEvents[name]['_id'];t.ajaxEvents[name]['_id'] +=1;t.ajaxEvents[name][id]=fct;return id},unregisterAjax:function (name,id){if (t.ajaxEvents[name]===undefined || t.ajaxEvents[name][id]===undefined) return false;t.ajaxEvents[name][id]=null;return true},ready:function (fct){var w=window,d=document,t=this;if (d.readyState==="complete"){fct();return}var done=false,top=true,root=d.documentElement,init=function (e){if (e.type=='readystatechange' && d.readyState !='complete') return;t.removeEvent((e.type=='load' ? w:d),e.type,init);if (!done && (done=true)) fct()},poll=function (){try{root.doScroll('left')}catch (e){setTimeout(poll,50);return}init('poll')};if (d.createEventObject && root.doScroll){try{top=!w.frameElement}if (top) poll()}t.addEvent(d,'DOMContentLoaded',init);t.addEvent(d,'readystatechange',init);t.addEvent(w,'load',init)},evalJSON:function (text,secure){if (typeof(text) !="string" || !text.length) return null;if (JSON !==undefined && typeof(JSON.parse)=='function'){try{var ret=JSON.parse(text);return ret}}\[\]0-9.\-+Eaeflnr-u \n\r\t]*$/).test(text.replace(/\\./g,'@').replace(/"[^"\\\n\r]*"/g, ''))) return null;
            try {
                var ret = eval('(' + text + ')');
                return ret;
            } catch (e) {
    ...
    var hikashop = {
        ...
        setup_chat: function () {
            var $ = jQuery;
            $(document).on("click", ".talking-to-suppliers", function () {
                var $self = jQuery(this);
                var admin_vendor_user_cms_id = $self.data('admin_vendor_user_cms_id');
                var $admin_vendor_user_cms = $("#jchat_userstab_popup").find('.jchat_userlist[data-loggedid="' + admin_vendor_user_cms_id + '"]');
                if($admin_vendor_user_cms.length>0){
                    $admin_vendor_user_cms.trigger('click');
                }else {
                    var list_user_cms_id = $self.data('list_user_cms_id');
                    list_user_cms_id = list_user_cms_id.split(',');
                    for (var i = 0; i < list_user_cms_id.length; i++) {
                        var user_cms_id = list_user_cms_id[i];
                        var $admin_vendor_user_cms = $("#jchat_userstab_popup").find('.jchat_userlist[data-loggedid="' + user_cms_id + '"]');
                        if($admin_vendor_user_cms.length>0){
                            $admin_vendor_user_cms.trigger('click');
                            break;
                        }
