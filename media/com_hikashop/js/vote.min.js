...
continue;window.hikaVotes[i].setRating(value,tooltip)}};hikaVote.vote=function(val,from){var d=document,re=new RegExp('id_(.*?)_hikashop'),m=re.exec(from),ref_id=null,infos=!1;if(m!=null){ref_id='';for(i=1;i<m.length;i++){ref_id=ref_id+m[i]+"\n"}}else{infos=d.querySelector("[data-ref][id=\"hikashop_vote_rating_id\"]");ref_id=infos.getAttribute("data-ref");if(!val&&infos.value){val=infos.value}}
...
var hikashop_vote_comment="",pseudo_comment=0,email_comment=0,recaptcha_comment='';if(d.getElementById("hikashop_vote_comment")){hikashop_vote_comment=d.getElementById("hikashop_vote_comment").value;pseudo_comment=d.getElementById("pseudo_comment").value;email_comment=d.getElementById("email_comment").value;if(d.getElementById("g-recaptcha-response")){recaptcha_comment=d.getElementById("g-recaptcha-response").value}}
...
return;data="data_id="+parseInt(ref_id);data+="&main_ctrl="+String(type);window.Oby.xRequest(hikaVote.options.urls.show,{mode:"POST",data:data},function(xhr){section.innerHTML=xhr.response})}
...
el.innerHTML=response.success.message});data="data_id="+parseInt(hikaVote.options.itemId)+"&main_ctrl="+encodeURIComponent(type)+"&content_type=listing";window.Oby.xRequest(hikaVote.options.urls.show,{mode:"POST",data:data},function(xhr){setTimeout(function(){section.innerHTML=xhr.response},5000)})};hikaVote.prototype={options:{},selectBox:null,container:null,max:null,cb:null,type:null,ref_id:null,init:function(el,opt,cb){var t=this,d=document;t.setOptions(opt);this.options.style='star';if(el.getAttribute('data-votestyle'))
this.options.style=el.getAttribute('data-votestyle');if(typeof(el)=='string')
t.selectBox=d.getElementById(el);else t.selectBox=el;if(el.voteInit)
...
t.selectBox.parentNode.insertBefore(t.container,t.selectBox.nextSibling);else t.selectBox.parentNode.appendChild(t.container)},reset:function(){if(t.container)
...
d.addEventListener(e,f,!1);else d['on'+e]=f;return f},hasClass:function(o,n){if(o.className=='')return!1;var reg=new RegExp("(^|\\s+)"+n+"(\\s+|$)");return reg.test(o.className)},addClass:function(o,n){if(o.className=='')
o.className=n;else if(!this.hasClass(o,n))
o.className+=' '+n},trim:function(s){return(s?''+s:'').replace(/^\s*|\s*$/g,'')},removeClass:function(e,c){var t=this;if(e.className!=''&&t.hasClass(e,c)){var cn=' '+e.className+' ';e.className=t.trim(cn.replace(' '+c+' ',' '))}}};var initVote=function(){var d=document,el=null,r=null;var voteContainers=d.getElementsByName('hikashop_vote_rating');if(voteContainers.length==0)
return;for(var i=0;i<voteContainers.length;i++){el=d.getElementById(voteContainers[i].id);if(!el.getAttribute("data-votetype"))
continue;r=new hikaVote(el,{id:'hikashop_vote_rating_'+el.getAttribute("data-votetype")+'_'+el.getAttribute("data-ref"),showSelectBox:!1,container:null,defaultRating:el.getAttribute("data-rate"),type:el.getAttribute("data-votetype"),ref_id:el.getAttribute("data-ref"),});if(r.container)
