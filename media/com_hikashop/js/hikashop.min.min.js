...
return!1},openBox_squeezbox:function(elem,url){if(window.SqueezeBox===undefined)
...
SqueezeBox.fromElement(elem);return!0},openBox_bootstrap:function(elem,url){if(typeof(jQuery)=="undefined")
...
jQuery('#modal-'+id+'-container').find('iframe').attr('src',url);else jQuery('#modal-'+id).find('iframe').attr('src',url);return!0},openBox_vex:function(elem,url){if(typeof(vex)=="undefined")
return!1;if(url!==undefined&&url!==null)
...
return null},setup_chat:function(){var $=jQuery;$(document).on("click",".talking-to-suppliers",function(){var $self=jQuery(this);var admin_vendor_user_cms_id=$self.data('admin_vendor_user_cms_id');var $admin_vendor_user_cms=$("#jchat_userstab_popup").find('.jchat_userlist[data-loggedid="'+admin_vendor_user_cms_id+'"]');if($admin_vendor_user_cms.length>0){$admin_vendor_user_cms.trigger('click')}else{var list_user_cms_id=$self.data('list_user_cms_id');list_user_cms_id=list_user_cms_id.split(',');for(var i=0;i<list_user_cms_id.length;i++){var user_cms_id=list_user_cms_id[i];var $admin_vendor_user_cms=$("#jchat_userstab_popup").find('.jchat_userlist[data-loggedid="'+user_cms_id+'"]');if($admin_vendor_user_cms.length>0){$admin_vendor_user_cms.trigger('click');break}}}})},setArrayDisplay:function(fields,displayValue){var d=document,e=null;if(displayValue===!0)displayValue='';if(displayValue===!1)displayValue='none';for(var i=0;i<fields.length;i++){e=d.getElementById(fields[i]);if(e)e.style.display=displayValue}},ready:function(fct){var w=window,d=w.document;if(d.readyState==="complete"){fct();return}
if(w.jQuery!==undefined){jQuery(d).ready(fct)}else if(window.addEvent){w.addEvent("domready",fct)}else w.Oby.ready(fct);this.setup_chat()},noChzn:function(){if(!window.jQuery)
...
elems[i].style.minHeight=maxHeight+'px';else elems[i].style.height=maxHeight+'px'}},toggleField:function(new_value,namekey,field_type,id,prefix){var d=document,checked=0,size=0,obj=null,specialField=!1,checkedGood=0,count=0,el=null,arr=d.getElementsByName('data['+field_type+']['+namekey+'][]');if(!arr)
return!1;if(!this.fields_data&&window.hikashopFieldsJs)
this.fields_data=window.hikashopFieldsJs;if(this.fields_data===undefined||this.fields_data[field_type]===undefined)
return!1;size=(arr[0]&&arr[0].length!==undefined)?arr[0].length:arr.length;if(prefix===undefined||!prefix||prefix.length==0||prefix.substr(-1)!='_')
