...
return _base}};$.zozo.core.plugins={easing:function(_base){var _exist=!1;if(_base){if(_base.settings){var _defEasing='swing';if($.easing.def){_exist=!0}
...
else{if(_base.settings.noTabs===!0){methods.showContent(_base,methods.getActive(_base,0))}
else{methods.showTab(_base,_base.settings.defaultTab)}}
...
else if(item&&content){tabOjb.tab=$("<li><a>"+item+"</a></li>");tabOjb.content=$("<div>"+content+"</div>");(tabID&&_base.settings.deeplinking===!0)&&(tabOjb.tab.attr(_base.settings.hashAttribute,tabID))}
if(tabOjb.tab&&tabOjb.content){tabOjb.tab.appendTo(_base.$tabGroup).hide().fadeIn(300).css("display","");tabOjb.content.appendTo(_base.$container);methods.updateClasses(_base);methods.bindEvent(_base,tabOjb.tab);setTimeout(function(){methods.checkWidth(_base,!1,!0)},350)}
...
{_tabToEnable.removeClass(DISABLEDCLASS);_tabToEnable.data(DISABLED,!1)}
...
return _base},select:function(_i){var _base=this;if(_base.settings.animating!==!0){if(_base.settings.noTabs===!0){methods.showContent(_base,methods.getActive(_base,_i))}
...
else{_base.settings.orientation=HORIZONTALCLASS;if(_isTop===!1){_base.$tabGroup.appendTo(_base.$elem);$(zozo.elementSpacer).appendTo(_base.$elem);_base.$container.prependTo(_base.$elem)}}
...
else{methods.ajaxRequest(_base,_tabElement)}}else{methods.showContent(_base,_tabElement)}}}},getActiveIndex:function(_base){var _index;if(_base.settings.noTabs===!0){_index=_base.$container.find(">div."+zozo.classes.active).index()}else{if(_base.currentTab)
...
_heightTotal=_heightTotal+$(this).innerHeight();_groupWidth=_groupWidth+_itemTempWidth});if(_init===!0)
...
var _isResORCompact=((methods.isCompact(_base)&&!methods.isMobile(_base)));var _css=(methods.isResponsive(_base)&&_base.BrowserDetection.isIE(7))?{"float":"none","width":"auto"}:{"float":""};var _hasResponsiveClass=_base.$elem.hasClass(RESPONSIVECLASS);_base.$tabs.each(function(index){if(((_hasResponsiveClass===!0&&(index+1)===_base.settings.original.itemD)||(index+1)===_base.settings.original.count)&&_isResORCompact){$(this).css(_css)}
...
if(_isTop===!0){(index===(_itemD-1))?_item.addClass(zozo.classes.right):_item.removeClass(zozo.classes.right)}
...
else{methods.showTab(_base,tab)}}},getFirst:function(_base){return 0},getLast:function(_base){if(_base.settings.noTabs===!0){return parseInt(_base.$container.children("div").size()-1)}
