!function(root,factory){if(typeof define==="function"&&define.amd)define(["jquery"],factory);else if(typeof exports==="object")module.exports=factory(require("jquery"));else factory(root.jQuery)}(this,function($){var cssPrefixes=["-webkit-","-moz-","-o-",""],eventPrefixes=["webkit","moz","MS","o",""];function prefixedEvent(element,name,callback){eventPrefixes.forEach(function(prefix){if(!prefix)name=name.toLowerCase();element.on(prefix+name,callback)})}function SimpleMarquee(element,options){this._element=
$(element);this._options=$.extend({speed:30,direction:"left",cycles:1,space:40,delayBetweenCycles:2E3,handleHover:true,handleResize:true,easing:"linear"},options);this._resizeDelay=parseInt(this._options.handleResize,10)||300;this._horizontal=this._options.direction==="left"||this._options.direction==="right";this._animationName="simplemarquee-"+Math.round(Math.random()*1E13).toString(18);this._onResize=this._onResize.bind(this);this._onCycle=this._onCycle.bind(this);this._options.handleResize&&$(window).on("resize",
this._onResize);this._options.handleHover&&this._element.on({"mouseenter.simplemarquee":this._onMouseEnter.bind(this),"mouseleave.simplemarquee":this._onMouseLeave.bind(this)});this._element.on("destroy.simplemarquee",this.destroy.bind(this));this.update(true)}SimpleMarquee.prototype.update=function(restart){this._reset();this._setup();if(!this._needsAnimation){this._paused=false;this._cycles=0}else if(restart){this._paused=false;this._cycles=-1;this._onCycle()}else if(this._paused)this._pause();
return this};SimpleMarquee.prototype.pause=function(){if(this._needsAnimation){this._resetCycle();if(!this._paused){this._pause();this._element.triggerHandler("pause");this._paused=true}}return this};SimpleMarquee.prototype.resume=function(){if(this._needsAnimation){this._resetCycle();if(this._paused){this._resume();this._element.triggerHandler("resume");this._paused=false}}return this};SimpleMarquee.prototype.toggle=function(){this._paused?this.resume():this.pause();return this};SimpleMarquee.prototype.destroy=
