...
function(event,ui){equal(ui.oldHeader.length,1);strictEqual(ui.oldHeader[0],headers[1]);equal(ui.oldPanel.length,1);strictEqual(ui.oldPanel[0],content[1]);equal(ui.newHeader.length,0);equal(ui.newPanel.length,0)});element.accordion("option","active",false);element.one("accordionbeforeactivate",function(event){ok(true);event.preventDefault()});element.one("accordionactivate",function(){ok(false)});element.accordion("option","active",1)})})(jQuery);
