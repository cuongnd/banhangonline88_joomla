...
var instanceOptions=instance.options=extend(!0,{},Class.defaults,Class.defaultOptions,options),element=instance.element=$(elem);if(Class.protoFactory&&(prototype=Class.protoFactory.apply(Class,[instance,instanceOptions,element]),extend(!0,instance,prototype)),(data(elem)||data(elem,{}))[_fullName]=instance,!element.data("directSelector")){var selector=$.uid("DS");element.addClass(selector).data("directSelector","."+selector)}for(name in instanceOptions)if(name.match(/^\{.+\}$/)){var key=name.replace(/^\{|\}$/g,""),val=instanceOptions[name];
...
break;case"iframe":callback.iframe=!0,callback.processData=!1,callback.files=options.data.files,delete options.data.files,options.data.transport="iframe"}delete callback.type}$.extend(options,callback)}$.isFunction(callback)&&(options.success=callback);var ajax=$.server(options);return ajax.progress(function(message,type,code){self.debug&&"debug"==type&&self.console.log(message,type,code)}),ajax}),$.Component.extend("Controller",function(){var staticProps,protoFactory,self=this,args=$.makeArray(arguments),name=args[0];
return 1==args.length?$.String.getObject(name):(args.length>2?(staticProps=args[1],protoFactory=args[2]):(staticProps={},protoFactory=args[1]),$.extend(staticProps,{root:self.className+".Controller",component:self}),$.Controller.apply(this,[name,staticProps,protoFactory]))}),$.Component.extend("Model",function(){var staticProps,protoFactory,self=this,args=$.makeArray(arguments),name=self.className+".Model."+args[0];return 1==args.length?$.String.getObject(args[0]):(2==args.length&&(staticProps={},protoFactory=args[1]),args.length>2&&(staticProps=args[1],protoFactory=args[2]),$.extend(staticProps,{component:self}),$.Model.apply(this,[name,staticProps,protoFactory]))
}),$.Component.extend("Model.List",function(){var staticProps,protoFactory,self=this,args=$.makeArray(arguments),name=self.className+".Model.List."+args[0];return 1==args.length?$.String.getObject(args[0]):(2==args.length&&(staticProps={},protoFactory=args[1]),args.length>2&&(staticProps=args[1],protoFactory=args[2]),$.extend(staticProps,{component:self}),$.Model.List.apply(this,[name,staticProps,protoFactory]))}),$.Component.extend("View",function(){var self=this;return arguments.length<1?self.template():(arguments[0]=self.prefix+arguments[0],$.View.apply(this,arguments))
}),FD40.module.execute(),$.each(FD40.component(),function(i,abstractComponent){abstractComponent.registered||$.Component.register(abstractComponent)})});