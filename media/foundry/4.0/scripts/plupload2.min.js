...
},addEventListener:function(type,fn,priority,scope){var list,self=this;return type=Basic.trim(type),/\s/.test(type)?(Basic.each(type.split(/\s+/),function(type){self.addEventListener(type,fn,priority,scope)}),void 0):(type=type.toLowerCase(),priority=parseInt(priority,10)||0,list=eventpool[this.uid]&&eventpool[this.uid][type]||[],list.push({fn:fn,priority:priority,scope:scope||this}),eventpool[this.uid]||(eventpool[this.uid]={}),eventpool[this.uid][type]=list,void 0)},hasEventListener:function(type){return type?!(!eventpool[this.uid]||!eventpool[this.uid][type]):!!eventpool[this.uid]
...
},1)}),runtime.bind("Error",function(){runtime.destroy(),initialize(items)}),runtime.mode?(runtime.init(),void 0):(runtime.trigger("Error"),void 0)):(initialize(items),void 0)):(comp.trigger("RuntimeError",new x.RuntimeError(x.RuntimeError.NOT_INIT_ERR)),runtime=null,void 0)}var ruid,comp=this;if("string"===Basic.typeOf(options)?ruid=options:"string"===Basic.typeOf(options.ruid)&&(ruid=options.ruid),ruid){if(runtime=Runtime.getRuntime(ruid))return runtime.clients++,runtime;throw new x.RuntimeError(x.RuntimeError.NOT_INIT_ERR)
...
}function XMLHttpRequest(){function _p(prop,value){return props.hasOwnProperty(prop)?1===arguments.length?Env.can("define_property")?props[prop]:self[prop]:(Env.can("define_property")?props[prop]=value:self[prop]=value,void 0):void 0}function _doXHR(data){function loadEnd(){_xhr&&(_xhr.destroy(),_xhr=null),self.dispatchEvent("loadend"),self=null}function exec(runtime){_xhr.bind("LoadStart",function(e){_p("readyState",XMLHttpRequest.LOADING),self.dispatchEvent("readystatechange"),self.dispatchEvent(e),_upload_events_flag&&self.upload.dispatchEvent(e)
...
break;case 4:_xhr.onreadystatechange=function(){},0===_xhr.status?target.trigger("error"):target.trigger("load")}},Basic.isEmptyObj(meta.headers)||Basic.each(meta.headers,function(value,header){_xhr.setRequestHeader(header,value)}),""!==meta.responseType&&"responseType"in _xhr&&(_xhr.responseType="json"!==meta.responseType||Env.can("return_response_type","json")?meta.responseType:"text"),mustSendAsBinary?_xhr.sendAsBinary?_xhr.sendAsBinary(data):function(){for(var ui8a=new Uint8Array(data.length),i=0;i<data.length;i++)ui8a[i]=255&data.charCodeAt(i);
...
}):_ep.setExif(tag,value),_hm.set("app1",_ep.getBinary()),void 0):!1},writeHeaders:function(){return arguments.length?_hm.restore(arguments[0]):_binstr=_hm.restore(_binstr)},stripHeaders:function(binstr){return _hm.strip(binstr)},purge:function(){_purge.call(this)}}),hasExif&&(this.meta={tiff:_ep.TIFF(),exif:_ep.EXIF(),gps:_ep.GPS()})}return JPEG}),define("moxie/runtime/html5/image/PNG",["moxie/core/Exceptions","moxie/core/utils/Basic","moxie/runtime/html5/utils/BinaryReader"],function(x,Basic,BinaryReader){function PNG(binstr){function _getDimensions(){var chunk,idx;
...
},fr.onerror=function(){comp.trigger("error",x.ImageError.WRONG_FORMAT)},fr.readAsDataURL(file),void 0):callback(file.getAsDataURL())}function _downsize(width,height,crop,preserveHeaders){var scale,img,destWidth,destHeight,orientation,self=this,x=0,y=0;if(_preserveHeaders=preserveHeaders,orientation=this.meta&&this.meta.tiff&&this.meta.tiff.Orientation||1,-1!==Basic.inArray(orientation,[5,6,7,8])){var tmp=width;width=height,height=tmp}return img=_getImg(),crop?(width=Math.min(width,img.width),height=Math.min(height,img.height),scale=Math.max(width/img.width,height/img.height)):scale=Math.min(width/img.width,height/img.height),scale>1&&!crop&&preserveHeaders?(this.trigger("Resize"),void 0):(_canvas||(_canvas=document.createElement("canvas")),destWidth=Math.round(img.width*scale),destHeight=Math.round(img.height*scale),crop?(_canvas.width=width,_canvas.height=height,destWidth>width&&(x=Math.round((destWidth-width)/2)),destHeight>height&&(y=Math.round((destHeight-height)/2))):(_canvas.width=destWidth,_canvas.height=destHeight),_preserveHeaders||_rotateToOrientaion(_canvas.width,_canvas.height,orientation),_drawToCanvas.call(this,img,_canvas,-x,-y,destWidth,destHeight),this.width=_canvas.width,this.height=_canvas.height,_modified=!0,self.trigger("Resize"),void 0)
...
break;case"filters":"array"===plupload.typeOf(value)&&(value={mime_types:value}),init?plupload.extend(settings.filters,value):settings.filters=value,value.mime_types&&(settings.filters.mime_types.regexp=function(filters){var extensionsRegExp=[];return plupload.each(filters,function(filter){plupload.each(filter.extensions.split(/,/),function(ext){/^\s*\*\s*$/.test(ext)?extensionsRegExp.push("\\.*"):extensionsRegExp.push("\\."+ext.replace(new RegExp("["+"/^$.*+?|()[]{}\\".replace(/./g,"\\$&")+"]","g"),"\\$&"))
})}),new RegExp("("+extensionsRegExp.join("|")+")$","i")}(settings.filters.mime_types));break;case"resize":init?plupload.extend(settings.resize,value,{enabled:!0}):settings.resize=value;break;case"prevent_duplicates":settings.prevent_duplicates=settings.filters.prevent_duplicates=!!value;break;case"browse_button":case"drop_element":value=plupload.get(value);case"container":case"runtimes":case"multi_selection":case"flash_swf_url":case"silverlight_xap_url":settings[option]=value,init||(reinitRequired=!0);break;
...
}),xhr.onload=function(){return xhr.status>=400?(handleError(),void 0):(retries=up.settings.max_retries,curChunkSize<blob.size?(chunkBlob.destroy(),offset+=curChunkSize,file.loaded=Math.min(offset,blob.size),up.trigger("ChunkUploaded",file,{offset:file.loaded,total:blob.size,response:xhr.responseText,status:xhr.status,responseHeaders:xhr.getAllResponseHeaders()}),"Android Browser"===o.Env.browser&&up.trigger("UploadProgress",file)):file.loaded=file.size,chunkBlob=formData=null,!offset||offset>=blob.size?(file.size!=file.origSize&&(blob.destroy(),blob=null),up.trigger("UploadProgress",file),file.status=plupload.DONE,up.trigger("FileUploaded",file,{response:xhr.responseText,status:xhr.status,responseHeaders:xhr.getAllResponseHeaders()})):delay(uploadNextChunk,1),void 0)
