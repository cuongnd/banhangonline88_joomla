...
var file=e.dataTransfer.files[0];return this.opts.dnbImageTypes!==!1&&-1==this.opts.dnbImageTypes.indexOf(file.type)?!0:(this.bufferSet(),this.showProgressBar(),this.opts.s3===!1?this.dragUploadAjax(this.opts.imageUpload,file,!0,e,this.opts.imageUploadParam):this.s3uploadFile(file),void 0)},buildEventPaste:function(e){var oldsafari=!1;if(this.browser("webkit")&&-1===navigator.userAgent.indexOf("Chrome")){var arr=this.browser("version").split(".");arr[0]<536&&(oldsafari=!0)}if(oldsafari)return!0;if(this.browser("opera"))return!0;
...
}if(this.imageResizeHide(!1),(parent&&"PRE"===$(parent).get(0).tagName||current&&"PRE"===$(current).get(0).tagName)&&(pre=!0,key===this.keyCode.DOWN&&this.insertAfterLastElement(block)),key===this.keyCode.DOWN&&(parent&&"BLOCKQUOTE"===$(parent)[0].tagName&&this.insertAfterLastElement(parent),current&&"BLOCKQUOTE"===$(current)[0].tagName&&this.insertAfterLastElement(current),parent&&"P"===$(parent)[0].tagName&&"BLOCKQUOTE"==$(parent).parent()[0].tagName&&this.insertAfterLastElement(parent,$(parent).parent()[0]),current&&"P"===$(current)[0].tagName&&parent&&"BLOCKQUOTE"==$(parent)[0].tagName&&this.insertAfterLastElement(current,parent)),this.shortcuts(e,key),ctrl&&90===key&&!e.shiftKey&&!e.altKey)return e.preventDefault(),this.opts.buffer.length?this.bufferUndo():this.document.execCommand("undo",!1,!1),void 0;
if(ctrl&&90===key&&e.shiftKey&&!e.altKey)return e.preventDefault(),0!=this.opts.rebuffer.length?this.bufferRedo():this.document.execCommand("redo",!1,!1),void 0;if(32==key&&this.bufferSet(),ctrl&&65===key?(this.bufferSet(),this.selectall=!0):key==this.keyCode.LEFT_WIN||ctrl||(this.selectall=!1),key!=this.keyCode.ENTER||e.shiftKey||e.ctrlKey||e.metaKey)key===this.keyCode.ENTER&&(e.ctrlKey||e.shiftKey)&&(this.bufferSet(),e.preventDefault(),this.insertLineBreak());else{var range=this.getRange();if(range&&range.collapsed===!1&&(sel=this.getSelection(),sel.rangeCount&&range.deleteContents()),this.browser("msie")&&1==parent.nodeType&&("TD"==parent.tagName||"TH"==parent.tagName))return e.preventDefault(),this.bufferSet(),this.insertNode(document.createElement("br")),this.callback("enter",e),!1;
...
},placeholderRemoveFromEditor:function(){this.$editor.find("span.redactor_placeholder").remove(),this.$editor.off("focus.redactor_placeholder")},placeholderRemoveFromCode:function(html){return html.replace(/<span class="redactor_placeholder"(.*?)>(.*?)<\/span>/i,"")},shortcuts:function(e,key){return this.opts.shortcuts?($.each(this.opts.shortcuts,$.proxy(function(str,command){var keys=str.split(",");for(var i in keys)this.shortcutsHandler(e,$.trim(keys[i]),$.proxy(function(){eval(command)},this))},this)),void 0):(!e.ctrlKey&&!e.metaKey||66!==key&&73!==key||e.preventDefault(),!1)
...
}},buttonAddBefore:function(beforekey,key,title,callback,dropdown){if(this.opts.toolbar){var btn=this.buttonBuild(key,{title:title,callback:callback,dropdown:dropdown},!0),$btn=this.buttonGet(beforekey);return 0!==$btn.size()?$btn.parent().before($("<li>").append(btn)):this.$toolbar.append($("<li>").append(btn)),btn}},buttonRemove:function(key){var $btn=this.buttonGet(key);$btn.remove()},buttonActiveObserver:function(e,btnName){var parent=this.getParent();if(this.buttonInactiveAll(btnName),e===!1&&"html"!==btnName)return-1!=$.inArray(btnName,this.opts.activeButtons)&&this.buttonActiveToggle(btnName),void 0;
parent&&"A"===parent.tagName?this.$toolbar.find("a.redactor_dropdown_link").text(this.opts.curLang.link_edit):this.$toolbar.find("a.redactor_dropdown_link").text(this.opts.curLang.link_insert),$.each(this.opts.activeButtonsStates,$.proxy(function(key,value){0!=$(parent).closest(key,this.$editor.get()[0]).length&&this.buttonActive(value)},this));var $parent=$(parent).closest(this.opts.alignmentTags.toString().toLowerCase(),this.$editor[0]);if($parent.length){var align=$parent.css("text-align");switch(align){case"right":this.buttonActive("alignright");
break;case"center":this.buttonActive("aligncenter");break;case"justify":this.buttonActive("alignjustify");break;default:this.buttonActive("alignleft")}}},execPasteFrag:function(html){var sel=this.getSelection();if(sel.getRangeAt&&sel.rangeCount){var range=this.getRange();range.deleteContents();var el=this.document.createElement("div");el.innerHTML=html;for(var node,lastNode,frag=this.document.createDocumentFragment();node=el.firstChild;)lastNode=frag.appendChild(node);frag.firstChild,range.insertNode(frag),lastNode&&(range=range.cloneRange(),range.setStartAfter(lastNode),range.collapse(!0)),sel.removeAllRanges(),sel.addRange(range)
...
("SUP"===parent.tagName||"SUB"===parent.tagName)&&this.inlineRemoveFormatReplace(parent)}return"inserthtml"===cmd?(this.insertHtml(param,sync),this.callback("execCommand",cmd,param),void 0):this.currentOrParentIs("PRE")&&!this.opts.formattingPre?!1:"insertunorderedlist"===cmd||"insertorderedlist"===cmd?this.execLists(cmd,param):"unlink"===cmd?this.execUnlink(cmd,param):(this.exec(cmd,param,sync),"inserthorizontalrule"===cmd&&this.$editor.find("hr").removeAttr("id"),void 0)},execUnlink:function(cmd,param){this.bufferSet();
var link=this.currentOrParentIs("A");return link?($(link).replaceWith($(link).text()),this.sync(),this.callback("execCommand",cmd,param),void 0):void 0},execLists:function(cmd,param){this.bufferSet();var parent=this.getParent(),$list=$(parent).closest("ol, ul");this.isParentRedactor($list)||0==$list.size()||($list=!1);var remove=!1;if($list&&$list.length){remove=!0;var listTag=$list[0].tagName;("insertunorderedlist"===cmd&&"OL"===listTag||"insertorderedlist"===cmd&&"UL"===listTag)&&(remove=!1)}if(this.selectionSave(),remove){var nodes=this.getNodes(),elems=this.getBlocks(nodes);
...
},this)),$(document).on("click.redactor",$.proxy(function(e){this.linkObserverTooltipClose(e)},this))},observeImages:function(){return this.opts.observeImages===!1?!1:(this.$editor.find("img").each($.proxy(function(i,elem){this.browser("msie")&&$(elem).attr("unselectable","on"),this.imageResize(elem)},this)),void 0)},linkObserver:function(e){var $link=$(e.target);if(0!=$link.size()&&"A"===$link[0].tagName){var pos=$link.offset();if(this.opts.iframe){var posFrame=this.$frame.offset();pos.top=posFrame.top+(pos.top-$(this.document).scrollTop()),pos.left+=posFrame.left
...
},this));tooltip.append(aLink),tooltip.append(" | "),tooltip.append(aEdit),tooltip.append(" | "),tooltip.append(aUnlink),tooltip.css({top:pos.top+20+"px",left:pos.left+"px"}),$(".redactor-link-tooltip").remove(),$("body").append(tooltip)}},linkObserverTooltipClose:function(e){return e!==!1&&"A"==e.target.tagName?!1:($(".redactor-link-tooltip").remove(),void 0)},getSelection:function(){return this.opts.rangy?this.opts.iframe?rangy.getSelection(this.$frame[0]):rangy.getSelection():this.document.getSelection()},getRange:function(){if(this.opts.rangy)return this.opts.iframe?rangy.createRange(this.iframeDoc()):rangy.createRange();
...
imageResizer!==!1&&(imageResizer.on("mousedown",function(e){isResizing=!0,e.preventDefault(),ratio=$image.width()/$image.height(),start_x=Math.round(e.pageX-$image.eq(0).offset().left),start_y=Math.round(e.pageY-$image.eq(0).offset().top)}),$(this.document.body).on("mousemove",$.proxy(function(e){if(isResizing){Math.round(e.pageX-$image.eq(0).offset().left)-start_x;var mouse_y=Math.round(e.pageY-$image.eq(0).offset().top)-start_y,div_h=$image.height(),new_h=parseInt(div_h,10)+mouse_y,new_w=Math.round(new_h*ratio);
new_w>min_w&&($image.width(new_w),100>new_w?this.imageEditter.css({marginTop:"-7px",marginLeft:"-13px",fontSize:"9px",padding:"3px 5px"}):this.imageEditter.css({marginTop:"-11px",marginLeft:"-18px",fontSize:"11px",padding:"7px 10px"})),start_x=Math.round(e.pageX-$image.eq(0).offset().left),start_y=Math.round(e.pageY-$image.eq(0).offset().top),this.sync()}},this)).on("mouseup",function(){isResizing=!1})),this.$editor.on("keydown.redactor-image-delete",$.proxy(function(e){var key=e.which;(this.keyCode.BACKSPACE==key||this.keyCode.DELETE==key)&&(this.bufferSet(!1),this.imageResizeHide(!1),this.imageRemove($image))
...
},this)),this.dropareabox.on("dragleave",$.proxy(function(){return this.draguploadOndragleave()},this)),this.dropareabox.get(0).ondrop=$.proxy(function(e){e.preventDefault(),this.dropareabox.removeClass("hover").addClass("drop"),this.showProgressBar(),this.dragUploadAjax(this.draguploadOptions.url,e.dataTransfer.files[0],!1,e,this.draguploadOptions.uploadParam)},this),void 0)},dragUploadAjax:function(url,file,directupload,e,uploadParam){if(!directupload){var xhr=$.ajaxSettings.xhr();xhr.upload&&xhr.upload.addEventListener("progress",$.proxy(this.uploadProgress,this),!1),$.ajaxSetup({xhr:function(){return xhr
...
},this)})},draguploadOndrag:function(){return this.dropareabox.addClass("hover"),!1},draguploadOndragleave:function(){return this.dropareabox.removeClass("hover"),!1},uploadProgress:function(e,text){var percent=e.loaded?parseInt(100*(e.loaded/e.total),10):e;this.dropareabox.text("Loading "+percent+"% "+(text||""))},isMobile:function(){return/(iPhone|iPod|BlackBerry|Android)/.test(navigator.userAgent)},isIPad:function(){return/iPad/.test(navigator.userAgent)},normalize:function(str){return"undefined"==typeof str?0:parseInt(str.replace("px",""),10)
...
}}}(jQuery)};exports(),module.resolveWith(exports)};FD40.module("redactor",moduleFactory)}();