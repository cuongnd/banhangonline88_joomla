...
return i===lastIndex?!1:(endColor=stops[i+1],startColor.stop!==endColor.stop&&(endStop=100-parseFloat(endColor.stop)+"%",startColor.octoHex=new Color(startColor.color).toIEOctoHex(),endColor.octoHex=new Color(endColor.color).toIEOctoHex(),filterVal="progid:DXImageTransform.Microsoft.Gradient(GradientType="+type+", StartColorStr='"+startColor.octoHex+"', EndColorStr='"+endColor.octoHex+"')",html+=template.replace("%start%",startColor.stop).replace("%end%",endStop).replace("%filter%",filterVal)),void 0)}),self.find(".iris-ie-gradient-shim").remove(),$(html).prependTo(self)
...
$(this).css({width:paletteWidth,height:paletteWidth,marginLeft:margin})}),self.picker.css("paddingBottom",paletteWidth+paletteMargin),strip.height(paletteWidth+paletteMargin+squareWidth),void 0):self.picker.css("paddingBottom","")},_addInputListeners:function(input){var self=this,debounceTimeout=100,callback=function(event){var color=new Color(input.val()),val=input.val().replace(/^#/,"");input.removeClass("iris-error"),color.error?""!==val&&input.addClass("iris-error"):color.toString()!==self._color.toString()&&("keyup"===event.type&&val.match(/^[0-9a-fA-F]{3}$/)||self._setOption("color",color.toString()))
...
}).on("keydown.palette",".iris-palette",function(event){return 13!==event.keyCode&&32!==event.keyCode?!0:(event.stopPropagation(),$(this).click(),void 0)})},_squareDrag:function(event,ui){var self=this,controlOpts=self.options.controls,dimensions=self._squareDimensions(),vertVal=Math.round((dimensions.h-ui.position.top)/dimensions.h*self._scale[controlOpts.vert]),horizVal=self._scale[controlOpts.horiz]-Math.round((dimensions.w-ui.position.left)/dimensions.w*self._scale[controlOpts.horiz]);self._color[controlOpts.horiz](horizVal)[controlOpts.vert](vertVal),self.active="square",self._change.apply(self,arguments)
