...
{ids:ids,attrs:attrs},success,error,"-updateAll","put")}},destroy:function(str){return function(ids,success,error){return ajax(str,ids,success,error,"-destroyAll","post")}}};$.Class($.globalNamespace+".Model.List",{setup:function(){for(var name in ajaxMethods)if(typeof this[name]!=="function")this[name]=ajaxMethods[name](this[name])}},{init:function(instances,noEvents){this.length=0;this._data={};this._namespace=".list"+ ++id,this.push.apply(this,$.makeArray(instances||[]))},slice:function(){return new this.Class(Array.prototype.slice.apply(this,
arguments))},match:function(property,value){return this.grep(function(inst){return inst[property]==value})},grep:function(callback,args){return new this.Class($.grep(this,callback,args))},_makeData:function(){var data=this._data={};this.each(function(i,inst){data[inst[inst.constructor.id]]=inst})},get:function(){if(!this.length)return new this.Class([]);if(this._changed)this._makeData();var list=[],constructor=this[0].constructor,underscored=constructor._fullName,idName=constructor.id,test=new RegExp(underscored+
"_([^ ]+)"),matches,val,args=getArgs(arguments);for(var i=0;i<args.length;i++){if(args[i].nodeName&&(matches=args[i].className.match(test)))val=this._data[matches[1]];else val=this._data[typeof args[i]=="string"||typeof args[i]=="number"?args[i]:args[i][idName]];val&&list.push(val)}return new this.Class(list)},remove:function(args){if(!this.length)return[];var list=[],constructor=this[0].constructor,underscored=constructor._fullName,idName=constructor.id,test=new RegExp(underscored+"_([^ ]+)"),matches,
val;args=getArgs(arguments);var i=0;while(i<this.length){var inst=this[i],found=false;for(var a=0;a<args.length;a++){var id=args[a].nodeName&&(matches=args[a].className.match(test))&&matches[1]||(typeof args[a]=="string"||typeof args[a]=="number"?args[a]:args[a][idName]);if(inst[idName]==id){list.push.apply(list,this.splice(i,1));args.splice(a,1);found=true;break}}if(!found)i++}var ret=new this.Class(list);if(ret.length)$([this]).trigger("remove",[ret]);return ret},elements:function(context){return $(this.map(function(item){return"."+
item.identity()}).join(","),context)},model:function(){return this.constructor.namespace},findAll:function(params,success,error){var self=this;this.model().findAll(params,function(items){self.push(items);success&&success(self)},error)},destroy:function(success,error){var ids=this.map(getIds),items=this.slice(0,this.length);if(ids.length)this.constructor.destroy(ids,function(){each(items,function(){this.destroyed()});success&&success(items)},error);else success&&success(this);return this},update:function(attrs,
success,error){var ids=this.map(getIds),items=this.slice(0,this.length);if(ids.length)this.constructor.update(ids,attrs,function(newAttrs){var attributes=$.extend(attrs,newAttrs||{});each(items,function(){this.updated(attributes)});success&&success(items)},error);else success&&success(this);return this},bind:function(){if(this[expando]===undefined)this.bindings(this);$.fn.bind.apply($([this]),arguments);return this},unbind:function(){$.fn.unbind.apply($([this]),arguments);if(this[expando]===undefined)$(this).unbind(this._namespace);
return this},bindings:function(items){var self=this;$(items).bind("destroyed"+this._namespace,function(){self.remove(this)}).bind("updated"+this._namespace,function(){$([self]).trigger("updated",this)})},push:function(){var args=getArgs(arguments);if(this[expando]!==undefined)this.bindings(args);this._changed=true;var res=push.apply(this,args);if(this[expando]&&args.length)$([this]).trigger("add",[args]);return res},serialize:function(){return this.map(function(item){return item.serialize()})}});
var push=[].push,modifiers={pop:[].pop,shift:[].shift,unshift:[].unshift,splice:[].splice,sort:[].sort,reverse:[].reverse};each(modifiers,function(name,func){$.Model.List.prototype[name]=function(){this._changed=true;return func.apply(this,arguments)}});each(["each","map"],function(i,name){$.Model.List.prototype[name]=function(callback,args){return $[name](this,callback,args)}})};exports();module.resolveWith(exports)})};dispatch("mvc/model.list").containing(moduleFactory).to("Foundry/2.1 Modules")})();
