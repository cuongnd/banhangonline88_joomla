...
class SocialGroupAppVideos extends SocialAppItem
{
    ...
    public function onPrepareStoryPanel(SocialStory $story)
    {
        ...
        if (!$adapter->canUpload() && !$adapter->canEmbed() || !$group->getCategory()->getAcl()->get('videos.create', true)) {
            return;
        }
    ...
    public function onAfterLikeSave(&$likes)
    {
        ...
        if ($likes->created_by != $video->user_id) {
            ES::notify('likes.item', array($video->user_id), $emailOptions, $systemOptions, $cluster->notification);
        }
