	...
	public function execute(&$item)
	{
		...
		if ($item->context_type == 'videos.event.create' || $item->context_type == 'videos.event.featured') {
			...
			if ($video->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
				$langString = FD::string()->computeNoun('APP_EVENT_VIDEOS_NOTIFICATIONS_LIKES_USER_VIDEO', count($users));
			...
			if ($item->actor_id == $video->user_id && count($users) == 1) {
				$item->title = JText::sprintf('APP_EVENT_VIDEOS_NOTIFICATIONS_LIKES_USERS_VIDEO' . ES::user($item->actor_id)->getGenderLang(), ES::user($item->actor_id)->getName());

				return;
			}

			// For other users, we just post a generic message
			$langString = ES::string()->computeNoun('APP_EVENT_VIDEOS_NOTIFICATIONS_LIKES_USERS_VIDEO', count($users));
