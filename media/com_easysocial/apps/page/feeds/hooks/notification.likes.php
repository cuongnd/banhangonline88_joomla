...
class SocialPageAppFeedsHookNotificationLikes
{
    /**
    ...
    public function execute(&$item)
    {
        ...
        if ($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
            $langString = ES::string()->computeNoun('APP_PAGE_FEEDS_NOTIFICATIONS_USER_LIKES_FEED', count($users));
        ...
        $langString = ES::string()->computeNoun('APP_USER_FEEDS_NOTIFICATIONS_USER_LIKES_USERS_FEED', count($users));
        $item->title = JText::sprintf($langString, $names, $page->getName());
