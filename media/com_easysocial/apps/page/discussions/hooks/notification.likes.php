...
class SocialPageAppDiscussionsHookNotificationLikes
{
    /**
    ...
    public function execute(&$item)
    {
        ...
        if ($item->context_type == 'discussions.page.create') {
            ...
            if ($discussion->created_by == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $langString = ES::string()->computeNoun('APP_PAGE_DISCUSSIONS_USER_LIKES_DISCUSSION', count($users));
            ...
            $langString = ES::string()->computeNoun('APP_PAGE_DISCUSSIONS_USER_LIKES_USERS_DISCUSSION', count($users));
            $item->title = JText::sprintf($langString, $names, $page->getName());
