...
class SocialPageAppNewsHookNotificationLikes
{
    /**
    ...
    public function execute(&$item)
    {
        ...
        if ($item->context_type == 'news.page.create') {
            ...
            if ($news->created_by == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $langString = ES::string()->computeNoun('APP_PAGE_NEWS_USER_LIKES_ANNOUNCEMENT', count($users));
            ...
            $langString = ES::string()->computeNoun('APP_PAGE_NEWS_USER_LIKES_USER_ANNOUNCEMENT', count($users));
            $item->title = JText::sprintf($langString, $names, ES::user($news->created_by)->getName(), $page->getName());
