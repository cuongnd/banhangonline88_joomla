...
return!0},registerAjax:function(name,fct){var t=this;if(typeof(name)=='object'){var r=[];for(var k=name.length-1;k>=0;k--){r[r.length]=t.registerAjax(name[k],fct)}
return r}
...
return null},getXHR:function(){var xhr=null,w=window;if(w.XMLHttpRequest||w.ActiveXObject){if(w.ActiveXObject){try{xhr=new ActiveXObject("Microsoft.XMLHTTP")}catch(e){}}else xhr=new w.XMLHttpRequest()}
...
return ret},encodeFormData:function(data){var ret='',v=null;if(typeof(data)=="string")
...
var hikamarket={submitFct:null,submitBox:function(data){var t=this;if(t.submitFct){try{t.submitFct(data)}catch(err){}}
t.closeBox()},deleteId:function(id){return window.hikashop.deleteId(id)},dup:function(tplName,htmlblocks,id,extraData,appendTo){return window.hikashop.dup(tplName,htmlblocks,id,extraData,appendTo)},deleteRow:function(id){return window.hikashop.deleteRow(id)},dupRow:function(tplName,htmlblocks,id,extraData){return window.hikashop.dupRow(tplName,htmlblocks,id,extraData)},cleanTableRows:function(id){return window.hikashop.cleanTableRows(id)},checkRow:function(id){return window.hikashop.checkRow(id)},isChecked:function(id,cancel){return window.hikashop.isChecked(id,cancel)},checkAll:function(checkbox,stub){stub=stub||'cb';if(!checkbox.form)
return!1;var o=window.Oby,cb=checkbox.form,c=0;for(var i=0,n=cb.elements.length;i<n;i++){var e=cb.elements[i];if(e!=checkbox&&e.type==checkbox.type&&((stub&&e.id.indexOf(stub)==0)||!stub)){e.checked=checkbox.checked;o.fireEvent(e,'change');c+=(e.checked==!0?1:0)}}
if(cb.boxchecked)
cb.boxchecked.value=c;return!0},submitform:function(task,form,extra){return window.hikashop.submitform(task,form,extra)},get:function(elem,target){return window.hikashop.get(elem,target)},form:function(elem,target){return window.hikashop.form(elem,target)},openBox:function(elem,url,jqmodal){return window.hikashop.openBox(elem,url,jqmodal)},closeBox:function(parent){return window.hikashop.closeBox(parent)},tabSelect:function(m,c,id){return window.hikashop.tabSelect(mc,id)},getOffset:function(el){return window.hikashop.getOffset(el)},switchBlock:function(el,values,name){var dest=document.getElementById(name);if(!dest)return;if(typeof(values)=='number')values=[values];for(var i=values.length-1;i>=0;i--){if(values[i]==el.value){dest.style.display='';return}}
dest.style.display='none'},dataStore:function(name,value){if(localStorage){localStorage.setItem(name,value)}else{var expire=new Date();expire.setDate(expire.getDate()+5);document.cookie=name+"="+value+"; expires="+expire}},dataGet:function(name){if(localStorage){return localStorage.getItem(name)}
...
return null},setArrayDisplay:function(fields,displayValue){var d=document,e=null;if(displayValue===!0)displayValue='';if(displayValue===!1)displayValue='none';for(var i=0;i<fields.length;i++){e=d.getElementById(fields[i]);if(e)e.style.display=displayValue}},dlTitle:function(parent){var t=this,d=document;if(parent&&typeof(parent)=='string')
parent=d.getElementById(parent);if(!parent)
parent=d;var dt=d.getElementsByTagName('dt'),val=null,hkTip=(typeof(hkjQuery)!="undefined"&&hkjQuery().hktooltip);for(var i=0;i<dt.length;i++){if(dt[i].offsetWidth<dt[i].scrollWidth&&!dt[i].getAttribute('title')){val=(dt[i].innerText!==undefined)?dt[i].innerText:dt[i].textContent;if(hkTip){dt[i].setAttribute('data-title',val);hkjQuery(dt[i]).hktooltip({"html":!0,"container":"body"})}else dt[i].setAttribute('title',val)}}},noChzn:function(){if(!window.jQuery)
...
dest.style.display='';o.addClass(el.parentNode,'active');el.blur();return!1},callbackFct:function(t,url,keyword,tree,node,ev){return treeCallbackFct(t,url,keyword,tree,node,ev)}};window.hikamarket=hikamarket})()