...
$tab_active = $session->get(mod_tabs_helper::MOD_TABS_TAB_ACTIVE . $module->id, 0);
?>
<div class="mod_tabs" id="mod_tabs_<?php echo $module->id ?>">
    <?php if ($module->showtitle): ?><h3 class="module-title"> <?php if ($current_module_image != ''): ?>
        <img class="icon" src="<?php echo JUri::root() . $current_module_image ?>"><?php endif; ?><?php echo $module->title ?>
        </h3> <?php endif; ?>
    <div class="tabs" id="tabbed-nav-<?php echo $module->id ?>">
                    ...
                    switch ($item_element[0]) {
                        case "module":
                            ...
                            if ($item_module) {
                                ...
                                ?>
                                <li><a><?php if ($module_image != ''): ?>
                                            <img class="icon" src="<?php echo JUri::root() . $module_image ?>"><?php endif; ?> <?php echo $item_module->title ?>
                                    </a></li>
                                <?php
                            }
                            break;
        ...
        <div>
            <?php
            if (count($jv_selection)) {
                foreach ($jv_selection as $key => $item_element) {
                    ?>
                    <div class="ajax-loading" <?php echo $tab_active==$key?' data-ajax_completed="true" ':'' ?> >
                        <?php
                        if($tab_active==$key) {
                            $item_element = explode('_', $item_element);
                            switch ($item_element[0]) {
