...
if (!class_exists('HKMinicartproBaseHelper')){
	/**
	 * BaseHelper for com_zoo only.
	 */
	abstract class HKMinicartproBaseHelper{
		/**
		...
		*/
		 ...
		 */
		public static function getPHKImage($item, $params, $prefix='imgcfg'){
			$images = self::getPHKImages($item, $params, $prefix);
		 ...
		 * @param unknown_type $params
		 */
		public static function getPHKImages($item, $params, $prefix='imgcfg'){
			...
			if ( !isset(self::$image_cache[$hash][$item->id]) ){
				...
				$images_path = array();
				$priority = preg_split('/[\s|,|;]/', $params->get($prefix.'_order', 'external, product_image,product_description'), -1, PREG_SPLIT_NO_EMPTY);
				if ( count($priority) > 0 ){
					...
					foreach($mark as $type => $true){
						switch ($type){
							case 'product_image':
								...
								$path = $config->get('uploadfolder');
								$pimages = $item->_image;
								if($pimages != '') {
									for ($i=0; $i<count($pimages); $i++){
										$image[$i] = $path.$pimages[$i]->file_path;
										if(file_exists($image[$i]) || @getimagesize($image[$i]) !== false) {
											array_push($images_path, $image[$i]);
										}
									}
								}
								break;
							case 'product_description':
								$text = isset($item->product_description) ? $item->product_description : '';
								...
								break;
							case 'external':
								$exf = $params->get($prefix.'_external_url', 'images/product/{id}/');
		...
		public static function getCHKImage($item, $params, $prefix='imgcfgcat'){
			$images = self::getCHKImages($item, $params, $prefix);
			return is_array($images) && count($images) ? $images[0] : null;
		}

		public static function getCHKImages($item, $params, $prefix='imgcfgcat'){
			...
			if ( !isset(self::$image_cache[$hash][$item->id]) ){
				...
				$images_path = array();
				$priority = preg_split('/[\s|,|;]/', $params->get($prefix.'_order', 'external, category_image, category_description'), -1, PREG_SPLIT_NO_EMPTY);
				if ( count($priority) > 0 ){
					$priority = array_map('strtolower', $priority);
					$mark = array();
					...
					foreach($mark as $type => $true){
						switch ($type){
							case 'category_image':
								$config = hikashop_config();
								$path = $config->get('uploadfolder');
								$pimages = $item->file_path;
								if($pimages != '') {
									for ($i=0; $i<count($pimages); $i++){
										$image[$i] = $path.$pimages[$i];
										if(file_exists($image[$i]) || @getimagesize($image[$i]) !== false) {
											array_push($images_path, $image[$i]);
										}
									}
							...
							case 'category_description':
								$text = isset($item->category_description) ? $item->category_description : '';
								$inline_images = self::getInlineImages($text);
								for ($i=0; $i<count($inline_images); $i++){
									if(file_exists($inline_images[$i]['src']) || @getimagesize($inline_images[$i]['src']) !== false) {
										array_push($images_path, $inline_images[$i]);
