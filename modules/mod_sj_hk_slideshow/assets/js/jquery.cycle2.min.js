/*
 ...
 Cycle2 autoheight plugin; Copyright (c) M.Alsup, 2012; version: 20130913  caption plugin for Cycle2;  version: 20130306  command plugin for Cycle2;  version: 20130707  hash plugin for Cycle2;  version: 20130905  loader plugin for Cycle2;  version: 20131121  pager plugin for Cycle2;  version: 20130525  prevnext plugin for Cycle2;  version: 20130709  progressive loader plugin for Cycle2;  version: 20130315  tmpl plugin for Cycle2;  version: 20121227 */
(function($){var version="2.0.2";$.fn.cycle=function(options){var o;if(this.length===0&&!$.isReady){o={s:this.selector,c:this.context};$.fn.cycle.log("requeuing slideshow (dom not ready)");$(function(){$(o.s,o.c).cycle(options)});return this}return this.each(function(){var data,opts,shortName,val;var container=$(this);var log=$.fn.cycle.log;if(container.data("cycle.opts"))return;if(container.data("cycle-log")===false||options&&options.log===false||opts&&opts.log===false)log=$.noop;log("--c2 init--");
...
initSlide:function(slideOpts,slide,suggestedZindex){var opts=this.opts();slide.css(slideOpts.slideCss||{});if(suggestedZindex>0)slide.css("zIndex",suggestedZindex);if(isNaN(slideOpts.speed))slideOpts.speed=$.fx.speeds[slideOpts.speed]||$.fx.speeds._default;if(!slideOpts.sync)slideOpts.speed=slideOpts.speed/2;slide.addClass(opts.slideClass)},updateView:function(isAfter,isDuring,forceEvent){var opts=this.opts();if(!opts._initialized)return;var slideOpts=opts.API.getSlideOpts();var currSlide=opts.slides[opts.currSlide];
...
0)opts.API.trigger("cycle-update-view",[opts,slideOpts,currSlide,isAfter]);if(isAfter)opts.API.trigger("cycle-update-view-after",[opts,slideOpts,currSlide])},getComponent:function(name){var opts=this.opts();var selector=opts[name];if(typeof selector==="string")return/^\s*[\>|\+|~]/.test(selector)?opts.container.find(selector):$(selector);if(selector.jquery)return selector;return $(selector)},stackSlides:function(curr,next,fwd){var opts=this.opts();if(!curr){curr=opts.slides[opts.currSlide];next=opts.slides[opts.nextSlide];
fwd=!opts.reverse}$(curr).css("zIndex",opts.maxZ);var i;var z=opts.maxZ-2;var len=opts.slideCount;if(fwd){for(i=opts.currSlide+1;i<len;i++)$(opts.slides[i]).css("zIndex",z--);for(i=0;i<opts.currSlide;i++)$(opts.slides[i]).css("zIndex",z--)}else{for(i=opts.currSlide-1;i>=0;i--)$(opts.slides[i]).css("zIndex",z--);for(i=len-1;i>opts.currSlide;i--)$(opts.slides[i]).css("zIndex",z--)}$(next).css("zIndex",opts.maxZ-1)},getSlideIndex:function(el){return this.opts().slides.index(el)}};$.fn.cycle.log=function log(){if(window.console&&
console.log)console.log("[cycle2] "+Array.prototype.join.call(arguments," "))};$.fn.cycle.version=function(){return"Cycle2: "+version};function lowerCase(s){return(s||"").toLowerCase()}$.fn.cycle.transitions={custom:{},none:{before:function(opts,curr,next,fwd){opts.API.stackSlides(next,curr,fwd);opts.cssBefore={opacity:1,display:"block"}}},fade:{before:function(opts,curr,next,fwd){var css=opts.API.getSlideOpts(opts.nextSlide).slideCss||{};opts.API.stackSlides(curr,next,fwd);opts.cssBefore=$.extend(css,
{opacity:0,display:"block"});opts.animIn={opacity:1};opts.animOut={opacity:0}}},fadeout:{before:function(opts,curr,next,fwd){var css=opts.API.getSlideOpts(opts.nextSlide).slideCss||{};opts.API.stackSlides(curr,next,fwd);opts.cssBefore=$.extend(css,{opacity:1,display:"block"});opts.animOut={opacity:0}}},scrollHorz:{before:function(opts,curr,next,fwd){opts.API.stackSlides(curr,next,fwd);var w=opts.container.css("overflow","hidden").width();opts.cssBefore={left:fwd?w:-w,top:0,opacity:1,display:"block"};
opts.cssAfter={zIndex:opts._maxZ-2,left:0};opts.animIn={left:0};opts.animOut={left:fwd?-w:w}}}};$.fn.cycle.defaults={allowWrap:true,autoSelector:".cycle-slideshow[data-cycle-auto-init!=false]",delay:0,easing:null,fx:"fade",hideNonActive:true,loop:0,manualFx:undefined,manualSpeed:undefined,manualTrump:true,maxZ:100,pauseOnHover:false,reverse:false,slideActiveClass:"cycle-slide-active",slideClass:"cycle-slide",slideCss:{position:"absolute",top:0,left:0},slides:"> img",speed:500,startingSlide:0,sync:true,
timeout:4E3,updateView:0};$(document).ready(function(){$($.fn.cycle.defaults.autoSelector).cycle()})})(jQuery);
