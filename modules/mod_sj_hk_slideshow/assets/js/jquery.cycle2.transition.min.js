/*
 scrollVert transition plugin for Cycle2; version: 20121120  tile transition plugin for Cycle2; version: 20121120  shuffle transition plugin for Cycle2;  version: 20121120  progressive loader plugin for Cycle2; version: 20130315  swipe plugin for Cycle2; version: 20121120  caption plugin for Cycle2; version: 20130306  caption2 plugin for Cycle2; version: 20130306 */
(function($){$.fn.cycle.transitions.scrollVert={before:function(opts,curr,next,fwd){opts.API.stackSlides(opts,curr,next,fwd);var height=opts.container.css("overflow","hidden").height();opts.cssBefore={top:fwd?-height:height,left:0,opacity:1,display:"block"};opts.animIn={top:0};opts.animOut={top:fwd?height:-height}}}})(jQuery);
(function($){$.fn.cycle.transitions.tileSlide=$.fn.cycle.transitions.tileBlind={before:function(opts,curr,next,fwd){opts.API.stackSlides(curr,next,fwd);$(curr).show();opts.container.css("overflow","hidden");opts.tileDelay=opts.tileDelay||opts.fx=="tileSlide"?100:125;opts.tileCount=opts.tileCount||7;opts.tileVertical=opts.tileVertical!==false;if(!opts.container.data("cycleTileInitialized")){opts.container.on("cycle-destroyed",$.proxy(this.onDestroy,opts.API));opts.container.data("cycleTileInitialized",
...
tileHeight*(num-1)}opts.container.find(".cycle-tiles-container").remove();var animCSS;var tileCSS={left:0,top:0,overflow:"hidden",position:"absolute",margin:0,padding:0};if(vert)animCSS=opts.fx=="tileSlide"?{top:height}:{width:0};else animCSS=opts.fx=="tileSlide"?{left:width}:{height:0};var tilesContainer=$('<div class="cycle-tiles-container"></div>');tilesContainer.css({zIndex:$curr.css("z-index"),overflow:"visible",position:"absolute",top:0});tilesContainer.insertBefore(next);for(var i=0;i<num;i++){tile=
$("<div></div>").css(tileCSS).css({width:num-1===i?lastTileWidth:tileWidth,height:num-1===i?lastTileHeight:tileHeight,marginLeft:vert?i*tileWidth:0,marginTop:vert?0:i*tileHeight}).append($curr.clone().css({position:"relative",maxWidth:"none",width:$curr.width(),margin:0,padding:0,marginLeft:vert?-(i*tileWidth):0,marginTop:vert?0:-(i*tileHeight)}));tiles=tiles.add(tile)}tilesContainer.append(tiles);$curr.hide();$next.show().css("opacity",1);animateTile(fwd?0:num-1);opts._tileAniCallback=function(){$next.show();
$curr.hide();tilesContainer.remove();callback()};function animateTile(i){tiles.eq(i).animate(animCSS,{duration:opts.speed,easing:opts.easing,complete:function(){if(fwd?num-1===i:0===i)opts._tileAniCallback()}});setTimeout(function(){if(fwd?num-1!==i:0!==i)animateTile(fwd?i+1:i-1)},opts.tileDelay)}},stopTransition:function(opts){opts.container.find("*").stop(true,true);if(opts._tileAniCallback)opts._tileAniCallback()},onDestroy:function(e){var opts=this.opts();opts.container.find(".cycle-tiles-container").remove()}}})(jQuery);
(function($){$.fn.cycle.transitions.shuffle={transition:function(opts,currEl,nextEl,fwd,callback){$(nextEl).show();var width=opts.container.css("overflow","visible").width();var speed=opts.speed/2;var element=fwd?currEl:nextEl;opts=opts.API.getSlideOpts(fwd?opts.currSlide:opts.nextSlide);var props1={left:-width,top:15};var props2=opts.slideCss||{left:0,top:0};if(opts.shuffleLeft!==undefined)props1.left=props1.left+parseInt(opts.shuffleLeft,10)||0;else if(opts.shuffleRight!==undefined)props1.left=
width+parseInt(opts.shuffleRight,10)||0;if(opts.shuffleTop)props1.top=opts.shuffleTop;$(element).animate(props1,speed,opts.easeIn||opts.easing).queue("fx",$.proxy(reIndex,this)).animate(props2,speed,opts.easeOut||opts.easing,callback);function reIndex(nextFn){this.stack(opts,currEl,nextEl,fwd);nextFn()}},stack:function(opts,currEl,nextEl,fwd){var i,z;if(fwd){opts.API.stackSlides(nextEl,currEl,fwd);$(currEl).css("zIndex",1)}else{z=1;for(i=opts.nextSlide-1;i>=0;i--)$(opts.slides[i]).css("zIndex",z++);
for(i=opts.slideCount-1;i>opts.nextSlide;i--)$(opts.slides[i]).css("zIndex",z++);$(nextEl).css("zIndex",opts.maxZ);$(currEl).css("zIndex",opts.maxZ-1)}}}})(jQuery);
...
(function($){$.extend($.fn.cycle.defaults,{captionFxOut:"fadeOut",captionFxIn:"fadeIn",captionFxSel:undefined,overlayFxOut:"fadeOut",overlayFxIn:"fadeIn",overlayFxSel:undefined});$(document).on("cycle-bootstrap",function(e,opts){opts.container.on("cycle-update-view-before",update);opts.container.one("cycle-update-view-after",init)});function update(e,opts,slideOpts,currSlide,isAfter){if(opts.captionPlugin!=="caption2")return;$.each(["caption","overlay"],function(){var fxBase=this+"Fx",fx=opts[fxBase+
"Out"]||"hide",template=slideOpts[this+"Template"],el=opts.API.getComponent(this),sel=opts[fxBase+"Sel"],speed=opts.speed,animEl;if(opts.sync)speed=speed/2;animEl=sel?el.find(sel):el;if(el.length&&template){if(fx=="hide")speed=0;animEl[fx](speed,function(){var content=opts.API.tmpl(template,slideOpts,opts,currSlide);el.html(content);animEl=sel?el.find(sel):el;if(sel)animEl.hide();fx=opts[fxBase+"In"]||"show";animEl[fx](speed)})}else el.hide()})}function init(e,opts,slideOpts,currSlide,isAfter){if(opts.captionPlugin!==
"caption2")return;$.each(["caption","overlay"],function(){var template=slideOpts[this+"Template"],el=opts.API.getComponent(this);if(el.length&&template)el.html(opts.API.tmpl(template,slideOpts,opts,currSlide))})}})(jQuery);
