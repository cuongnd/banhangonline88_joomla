...
abstract class HKAjaxtabsHelper extends HKAjaxtabsBaseHelper
{
	public static function getList($params, $module)
	{
		...
		if (JRequest::getInt($preload_cookie, -1, 'cookie') != -1) {
			...
			if (in_array($preload_cookie_value, $_catids)) {
				$catid_loadfirst = $preload_cookie_value;
			}
		} else if (isset($catid_preload) && in_array($catid_preload, $_catids)) {
			$catid_loadfirst = (int)$catid_preload;
		}

		$_list = self::_getCatInfor($_catids, $params);
		if (empty($_list)) return;

		$list = array();
		foreach ($_list as $item) {
			if ($item->id == $catid_loadfirst) {
	...
	{
		if ($params->get('show_child_category_products', 0) && (int)$params->get('depth', 0) > 0) {
			$catids = self::_getChildCategory($catids, $params, true);
		}
