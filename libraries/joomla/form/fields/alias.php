 ...
 * @since  11.1
 */
class JFormFieldAlias extends JFormField
{
	/**
	 * The form field type.
	 *
	...
	protected function getInput()
	{
		...
		$input_name=(string)$this->element['input_name'];
		...
		$doc->addScript(JUri::root().'libraries/joomla/form/fields/alias.js');
		ob_start();
		$element_id="field_alias_$this->id";
		?>
		<script type="text/javascript">
			jQuery(document).ready(function ($) {
				$('#<?php echo $element_id ?>').field_alias({
		...
		<div id="<?php echo  $element_id ?>">
			<div class="input-group">
				<input type="text" name="<?php echo $this->name ?>" id="<?php echo $this->id ?>" <?php echo $dirname ?> value="<?php echo htmlspecialchars($this->value, ENT_COMPAT, 'UTF-8') ?>" <?php echo $class . $size . $disabled . $readonly . $list
					. $hint . $onchange . $maxLength . $required . $autocomplete . $autofocus . $spellcheck . $inputmode . $pattern ?> />
			  <span class="input-group-btn">
				<button class="btn btn-secondary auto-get-alias" type="button"><?php echo JText::_('Auto get alias') ?></button>
			  </span>
			</div>
			<?php echo $datalist ?>
		</div>
