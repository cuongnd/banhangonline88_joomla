...
*/
        ...
        plugin.addMessage = function(msg_item) {
            ...
            if($sub_wrapper_chat.length>0){
                var $msg=$('<div class="messenger-item"><b>'+(token==msg_item.token?list_messenger['ME']:msg_item.name)+'</b>:'+msg_item.msg+(token!=msg_item.token?'<a title="'+list_messenger['CREATE_ROOM_PRIVATE_CHATTING_WITH_THIS_USER']+'" href="javascript:void(0)"  class="create-room-private-chatting-with-this-user"><i class="icon ti-comment-alt"></i></a>':'')+'</div>');
