...
class JFormFieldFSJPluginParams extends JFormFieldText
{
	...
	{
		$plugin_id = $this->form->getValue('name');
		$plugin_type_id = $this->form->getValue('type');
		...
		$text = $xml->plugin_settings->asXML();
		$text = str_replace("<plugin_settings>", "", $text);
		$text = str_replace("</plugin_settings>", "", $text);

		/* get plugin parameters */
		$plugin = FSJ_Plugin_Handler::GetPlugin($plugin_type_id, $plugin_id);
		$xmlfile = JPATH_ROOT.DS.$plugin->path . DS . "{$plugin_type_id}.{$plugin_id}.xml";
		$xml = simplexml_load_file($xmlfile);
		
		$text2 = $xml->plugin_settings->asXML();
		$text2 = str_replace("<plugin_settings>", "", $text2);
		$text2 = str_replace("</plugin_settings>", "", $text2);
		
		$xmltext = "<form>" . $text . $text2 . "</form>";
		
		$form_xml = @simplexml_load_string($xmltext);
